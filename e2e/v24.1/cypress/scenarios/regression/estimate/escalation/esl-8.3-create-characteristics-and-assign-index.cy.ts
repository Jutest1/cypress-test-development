...
ALLURE.story("ESL- 8.3 | Create-characteristics-and-assign-index");

describe("ESL- 8.3 | Create-characteristics-and-assign-index", () => {
    before(function () {   
        cy.fixture("escalation/esl-8.3-create-characteristics-and-assign-index.json").then((data) => {
            this.data = data
    ...
    it("TC - Create characteristic for characteristic group", function () {
        ...
        cy.wait(2000) //need time to load data
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        cy.REFRESH_CONTAINER()
        _common.waitForLoaderToDisappear()
    ...
    it("TC - Assertion oF Index in Characterisitic", function () {

        _common.clear_subContainerFilter(cnt.uuid.CHARACTERISTIC_CHAR_GROUP)
        _common.search_inSubContainer(cnt.uuid.CHARACTERISTIC_CHAR_GROUP, CHAR_DESC)
        _common.select_rowHasValue(cnt.uuid.CHARACTERISTIC_CHAR_GROUP, CHAR_DESC)
        _common.assert_cellData_insideActiveRow(cnt.uuid.CHARACTERISTIC_CHAR_GROUP,app.GridCells.INDEX_HEADER_FK,CEMENT_DESC)
        _common.clear_subContainerFilter(cnt.uuid.CHARACTERISTIC_CHAR_GROUP)
        _common.search_inSubContainer(cnt.uuid.CHARACTERISTIC_CHAR_GROUP, CHAR_SANDDESC)
        _common.select_rowHasValue(cnt.uuid.CHARACTERISTIC_CHAR_GROUP, CHAR_SANDDESC)
        _common.assert_cellData_insideActiveRow(cnt.uuid.CHARACTERISTIC_CHAR_GROUP,app.GridCells.INDEX_HEADER_FK,SAND_DESC)
