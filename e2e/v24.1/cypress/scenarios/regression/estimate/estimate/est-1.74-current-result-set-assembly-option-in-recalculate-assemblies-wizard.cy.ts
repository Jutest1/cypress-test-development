...
let ASSEMBLY_RESOURCE_COST_TOTAL:string[]=[]
...
allure.story("EST- 1.74 | Current result set assembly option in recalculate assemblies wizard")

describe("EST- 1.74 | Current result set assembly option in recalculate assemblies wizard", () => {
    before(function () {
        cy.fixture("estimate/est-1.74-current-result-set-assembly-option-in-recalculate-assemblies-wizard.json").then((data) => {
        this.data = data
...
it('TC - Recalculate assembly and verify cost total and cost type ', function () {
    ...
    _common.assert_cellData_insideActiveRow(cnt.uuid.ASSEMBLY_RESOURCE, app.GridCells.EST_COST_TYPE_FK, CONTAINERS_MATERIAL_RECORD.COST_TYPE)
    ASSEMBLY_RESOURCE_COST_TOTAL=_common.returnArrayForMultipleCell(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.COST_TOTAL)
    _common.getText_fromCell(cnt.uuid.ASSEMBLIES,app.GridCells.COST_TOTAL)
    .then(($resourceCostTotal)=>{
     let COST_TOTAL:any=0
     for (var i in ASSEMBLY_RESOURCE_COST_TOTAL) {
        COST_TOTAL += (+parseFloat(ASSEMBLY_RESOURCE_COST_TOTAL[i].replace(',','')).toFixed(3));
     }
     expect(parseFloat($resourceCostTotal.text().replace(',','')).toFixed(2)).equal(parseFloat(COST_TOTAL).toFixed(2))
    })
})
