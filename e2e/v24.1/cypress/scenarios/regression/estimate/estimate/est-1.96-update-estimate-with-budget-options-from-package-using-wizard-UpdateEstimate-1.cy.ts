...
ALLURE.story("Est- 1.96 | Update Estimate with Budget options from Package using wizard 'Update Estimate' 1 ");
describe("Est- 1.96 | Update Estimate with Budget options from Package using wizard 'Update Estimate' 1", () => {
    before(function () {
        cy.fixture("estimate/est-1.96-update-estimate-with-budget-options-from-package-using-wizard-UpdateEstimate-1.json")
            .then((data) => {
                this.data = data;
    ...
    it("TC - Set Estimation type under customizing module", function () {
        ...
        _validate.customizing_DataRecordCheckBox(app.GridCells.IS_TOTAL_AQ_BUDGET, commonLocators.CommonKeys.CHECK)
