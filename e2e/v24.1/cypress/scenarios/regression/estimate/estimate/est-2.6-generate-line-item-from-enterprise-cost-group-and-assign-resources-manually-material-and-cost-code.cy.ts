...
ALLURE.story("EST- 2.6 | Generate Line item from Enterprise Cost group and assign resources manually (material and cost code)")

describe("EST- 2.6 | Generate Line item from Enterprise Cost group and assign resources manually (material and cost code)", () => {

    before(function () {
        cy.fixture("estimate/est-2.6-generate-line-item-from-enterprise-cost-group-and-assign-resources-manually-material-and-cost-code.json")
          .then((data) => {
            this.data = data;
	...
	it('TC - Add Cost group catalog', function () {
		...
		_common.enterRecord_inNewRow(cnt.uuid.COSTGROUP, app.GridCells.CODE, app.InputFields.DOMAIN_TYPE_CODE, SUB_COST_GROUP);
		_common.enterRecord_inNewRow(cnt.uuid.COSTGROUP, app.GridCells.DESCRIPTION_INFO, app.InputFields.DOMAIN_TYPE_TRANSLATION, SUB_COST_GROUP_DESC);
		_common.enterRecord_inNewRow(cnt.uuid.COSTGROUP, app.GridCells.QUANTITY_SMALL,app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_COST_GROUP.QUANTITY_1);
		_common.edit_dropdownCellWithInput(cnt.uuid.COSTGROUP, app.GridCells.UOM_FK,commonLocators.CommonKeys.GRID, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_COST_GROUP.UOM);
		_common.select_activeRowInContainer(cnt.uuid.COSTGROUP)
		_common.waitForLoaderToDisappear()
		cy.SAVE();
		...
		_common.clickOn_cellHasUniqueValue(cnt.uuid.COSTGROUP, app.GridCells.CODE, COST_GROUP);
		_common.create_newSubRecord(cnt.uuid.COSTGROUP);
		_common.enterRecord_inNewRow(cnt.uuid.COSTGROUP, app.GridCells.CODE, app.InputFields.DOMAIN_TYPE_CODE, SUB1_COST_GROUP);
		_common.enterRecord_inNewRow(cnt.uuid.COSTGROUP, app.GridCells.DESCRIPTION_INFO, app.InputFields.DOMAIN_TYPE_TRANSLATION,SUB1_COST_GROUP_DESC);
		_common.enterRecord_inNewRow(cnt.uuid.COSTGROUP, app.GridCells.QUANTITY_SMALL,app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_COST_GROUP.QUANTITY_2);
		_common.select_activeRowInContainer(cnt.uuid.COSTGROUP)
		_common.waitForLoaderToDisappear()
		cy.SAVE();
		_common.waitForLoaderToDisappear()
		_common.minimizeContainer(cnt.uuid.COSTGROUP)
	});
