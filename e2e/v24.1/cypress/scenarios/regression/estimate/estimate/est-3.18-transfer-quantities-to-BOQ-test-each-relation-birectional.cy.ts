...
ALLURE.story("EST- 3.18 | Transfer quantities to BOQ test relation bidirectional");

describe("EST- 3.18 | Transfer quantities to BOQ test relation bidirectional", () => {

  before(function () {
    cy.fixture("estimate/est-3.18- transfer-quantities-to-BOQ-test-relation-bidirectional.json")
      .then((data) => {
        this.data = data;
  ...
  after(() => {
	...
	it("TC - Create BOQ header and BOQ structure", function () {
		...
		_common.openTab(app.TabBar.BOQSTRUCTURE).then(() => {
			...
			_common.set_columnAtTop([CONTAINER_COLUMNS_BOQ_STRUCTURE.briefinfo,CONTAINER_COLUMNS_BOQ_STRUCTURE.quantity,CONTAINER_COLUMNS_BOQ_STRUCTURE.basuomfk,CONTAINER_COLUMNS_BOQ_STRUCTURE.price,CONTAINER_COLUMNS_BOQ_STRUCTURE.boqlinetypefk,CONTAINER_COLUMNS_BOQ_STRUCTURE.quantityadj,CONTAINER_COLUMNS_BOQ_STRUCTURE.pricegross],cnt.uuid.BOQ_STRUCTURES)
		});
		_boqPage.enterRecord_toCreateBoQStructure(cnt.uuid.BOQ_STRUCTURES,BOQ_STRUCTURE_PARAMETERS);
		cy.SAVE()
		_common.waitForLoaderToDisappear()
    _common.select_rowHasValue(cnt.uuid.BOQ_STRUCTURES,BOQ_STRUCTURE_DESC)
		_common.enterRecord_inNewRow(cnt.uuid.BOQ_STRUCTURES,app.GridCells.QUANTITY_ADJ,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_BOQ_STRUCTURE.AQ_QUANTITY)
		_common.select_activeRowInContainer(cnt.uuid.BOQ_STRUCTURES)
		_common.waitForLoaderToDisappear()
    _common.enterRecord_inNewRow(cnt.uuid.BOQ_STRUCTURES,app.GridCells.PRICE_GROSS,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_BOQ_STRUCTURE.CORRECTED_UR_GROSS)
		_common.select_activeRowInContainer(cnt.uuid.BOQ_STRUCTURES)
		_common.waitForLoaderToDisappear()
		cy.SAVE()
		_common.waitForLoaderToDisappear()
	...
	it("TC - Generate boq line item", function () {
		...
		_common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
			...
			_common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.quantitytarget,CONTAINER_COLUMNS_LINE_ITEM.estqtyrelboqfk],cnt.uuid.ESTIMATE_LINEITEMS)
		});
	...
	it('TC - Update BoQ quantities from wizard', function () {
		...
		_common.enterRecord_inNewRow(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.QUANTITY_SMALL,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_LINE_ITEM.QUANTITY_1)
		...
		_common.select_activeRowInContainer(cnt.uuid.ESTIMATE_LINEITEMS)
		_common.waitForLoaderToDisappear()
		_common.edit_dropdownCellWithCaret(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.EST_QTY_REL_BOQ_FK,commonLocators.CommonKeys.SPAN,commonLocators.CommonKeys.BIDIRECTIONAL_WQ_AQ);
		_common.select_activeRowInContainer(cnt.uuid.ESTIMATE_LINEITEMS)
		_common.waitForLoaderToDisappear()
		cy.SAVE()
		_common.waitForLoaderToDisappear()
	...
	it('TC - Verify updated quantity of BoQ structure - bidirectional', function () {
		_common.waitForLoaderToDisappear()
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART);
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT);
		_common.waitForLoaderToDisappear()
