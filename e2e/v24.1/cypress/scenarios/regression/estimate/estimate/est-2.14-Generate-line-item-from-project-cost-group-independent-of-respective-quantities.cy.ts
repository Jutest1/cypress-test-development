...
ALLURE.story("EST- 2.14 | Generate line item from project cost group independent of respective quantities");
describe("EST- 2.14 | Generate line item from project cost group independent of respective quantities", () => {

	before(function () {
		cy.fixture("estimate/est-2.14-Generate-line-item-from-project-cost-group-independent-of-respective-quantities.json")
		.then((data) => {
			this.data = data;
	...
	it('TC - Create new estimate record', function () {
  	...
  	it('TC - Verify Line items gets generated from Project Cost group', function () {
		...
		_common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
			...
			_common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.estqtytelaotfk,CONTAINER_COLUMNS_LINE_ITEM.quantitytarget],cnt.uuid.ESTIMATE_LINEITEMS)
		});
		_common.waitForLoaderToDisappear()
