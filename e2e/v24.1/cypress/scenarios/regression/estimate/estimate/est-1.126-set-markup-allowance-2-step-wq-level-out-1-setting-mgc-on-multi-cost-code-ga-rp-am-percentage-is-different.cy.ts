...
ALLURE.story("EST- 1.126 | Set Markup Allowance:2-step, WQ, level out=1;Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different;");

describe("EST- 1.126 | Set Markup Allowance:2-step, WQ, level out=1;Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different", () => {

    before(function () {

        cy.fixture("estimate/est-1.126-set-markup-allowance-2-step-wq-level-out-1-setting-mgc-on-multi-cost-code-ga-rp-am-percentage-is-different.json")
            .then((data) => {
                this.data = data;
