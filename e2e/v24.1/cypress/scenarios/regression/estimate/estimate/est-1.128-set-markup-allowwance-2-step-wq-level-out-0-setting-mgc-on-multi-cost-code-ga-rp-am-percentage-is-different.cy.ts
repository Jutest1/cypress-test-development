...
const LINE_ITEM_05 = "LINE-04-" + Cypress._.random(0, 999);
...
ALLURE.story("EST- 1.128 | Set Markup Allowance: 2-step, WQ, level out=0; Setting M(GC)[%] on multi costcode; GA RP AM percentage is different.");

describe("EST- 1.128 | Set Markup Allowance: 2-step, WQ, level out=0; Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different.", () => {

    before(function () {

        cy.fixture("estimate/est-1.128-set-markup-allowwance-2-step-wq-level-out-0-setting-mgc-on-multi-cost-code-ga-rp-am-percentage-is-different.json")
            .then((data) => {
                this.data = data;
