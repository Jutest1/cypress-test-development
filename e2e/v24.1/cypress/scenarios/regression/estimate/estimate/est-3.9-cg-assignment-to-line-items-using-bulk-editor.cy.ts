...
ALLURE.story("EST- 3.9 | CG assignment to line items using bulk editor");

describe("EST- 3.9 | CG assignment to line items using bulk editor", () => {

  before(function () {
    cy.fixture("estimate/est-3.9-cg-assignment-to-line-items-using-bulk-editor.json")
      .then((data) => {
        this.data = data;
  ...
  it("TC - Verify bulk editor records.", function () {
    _common.select_allContainerData(cnt.uuid.ESTIMATE_LINEITEMS);
    _validate.verify_bulkEditRecords(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.COST_GROUP_LIC_DIN276_2018_12, CONTAINERS_LINE_ITEM.COST_GROUP);
  });
