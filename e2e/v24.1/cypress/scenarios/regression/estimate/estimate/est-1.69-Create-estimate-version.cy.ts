...
ALLURE.story("EST- 1.69 | Create estimate version");

describe("EST- 1.69 | Create estimate version", () => {
  before(function () {

    cy.fixture("estimate/est-1.69-Create-estimate-version.json")
      .then((data) => {
        this.data=data
  ...
  it('TC - Verify cost/unit, and cost total of new estimate record', function () {
    _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
      _common.select_tabFromFooter(cnt.uuid.ESTIMATE_LINEITEMS, app.FooterTab.LINE_ITEMS, 0);
      _common.setup_gridLayout(cnt.uuid.ESTIMATE_LINEITEMS,CONTAINER_COLUMNS_LINE_ITEM )
      _common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.costtotal,CONTAINER_COLUMNS_LINE_ITEM.costunit,CONTAINER_COLUMNS_LINE_ITEM.quantity],cnt.uuid.ESTIMATE_LINEITEMS)
    });
    _common.maximizeContainer(cnt.uuid.ESTIMATE_LINEITEMS)
    _common.clear_subContainerFilter(cnt.uuid.ESTIMATE_LINEITEMS)
    _common.select_rowHasValue(cnt.uuid.ESTIMATE_LINEITEMS,BOQ_STRUCTURE_DESC)
    _common.assert_forNumericValues(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.COST_TOTAL,Cypress.env("COST_TOTAL"))
    _validate.verify_inputFieldVisibility(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.QUANTITY_SMALL,commonLocators.CommonKeys.INPUT_SHOULD_NOT_EXIST,app.InputFields.INPUT_GROUP_CONTENT)
    _validate.verify_inputFieldVisibility(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.COST_UNIT,commonLocators.CommonKeys.INPUT_SHOULD_NOT_EXIST,app.InputFields.INPUT_GROUP_CONTENT)
  });
});
