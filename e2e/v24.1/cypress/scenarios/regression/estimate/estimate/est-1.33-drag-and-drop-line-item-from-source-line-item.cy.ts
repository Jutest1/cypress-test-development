import { _common, _estimatePage, _validate, _mainView, _boqPage, _modalView, _projectPage } from "cypress/pages";
...
ALLURE.story("EST- 1.33 | Drag and drop line item from Source Line item");

describe("EST- 1.33 | Drag and drop line item from Source Line item", () => {

  before(function () {
      cy.fixture("estimate/est-1.33-drag-and-drop-line-item-from-source-line-item.json")
        .then((data) => {
          this.data=data
  ...
  it("TC - Create new line item record", function () {
    ...
    cy.SAVE()
    _common.waitForLoaderToDisappear()
    _common.saveCellDataToEnv(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.CODE,"LINE_ITEM_CODE")
  ...
  it("TC - Verify drag and drop source line item to line item", function () {
      ...
      _common.clear_subContainerFilter(cnt.uuid.ESTIMATE_LINEITEMS);
      _validate.verify_DragAndDropEstimateLineItem(Cypress.env("LINE_ITEM_CODE"),CONTAINERS_LINE_ITEM.QUANTITY)
  });
