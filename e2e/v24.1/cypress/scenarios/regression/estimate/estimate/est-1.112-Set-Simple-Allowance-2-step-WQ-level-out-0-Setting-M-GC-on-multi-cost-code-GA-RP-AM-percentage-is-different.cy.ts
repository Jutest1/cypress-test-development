...
import { _common, _projectPage, _boqPage, _estimatePage, _validate } from "cypress/pages";
...
ALLURE.story("EST- 1.112 | Set Simple Allowance: Change to  2-step, WQ, level out=  0; Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different");

describe("EST- 1.112 | Set Simple Allowance: Change to  2-step, WQ, level out=  0; Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different", () => {

    before(function () {
        cy.fixture("estimate/est-1.112-Set-Simple-Allowance-2-step-WQ-level-out-0-Setting-M-GC-on-multi-cost-code-GA-RP-AM-percentage-is-different.json")
            .then((data) => {
                this.data = data;
                ...
                _common.waitForLoaderToDisappear()
                _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_SEARCH).delete_pinnedItem();
                _common.search_fromSidebar(commonLocators.CommonKeys.STANDARD, "PRJ-P-976_697").pinnedItem();
            });
    });
    ...
    it('TC - Verify create standard allowance', function () {
        ...
        _common.waitForLoaderToDisappear()
        _common.set_cellCheckboxValue(cnt.uuid.STANDARD_ALLOWANCES, app.GridCells.IS_ONE_STEP, commonLocators.CommonKeys.UNCHECK)
        cy.SAVE();
        _common.waitForLoaderToDisappear()
    });
    ...
    it('TC - Add Allowance/Unit and Grand Cost/unit', function () {
        ...
        _common.select_rowInContainer(cnt.uuid.STANDARD_ALLOWANCES)
    ...
    it('TC - Verify GA,GC,RP In Resource container', function () {
        ...
        _common.clickOn_cellHasUniqueValue(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.DESCRIPTION_INFO, "BOQ-IT1-435363663")
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
            _common.select_tabFromFooter(cnt.uuid.RESOURCES, app.FooterTab.RESOURCES, 1);
        });
