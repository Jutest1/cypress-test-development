    ...
    let ASSEMBLIES_PARAMETERS1:DataCells
    ...
    allure.story("EST- 1.89 | Part-1 Update project assembly with current result set option ")
    
    describe('EST- 1.89 | Part-1 Update project assembly with current result set option', () => {
            before(function () {
                cy.fixture('estimate/est-1.89-Update_project_assembly_with_current_result_set_option.json')
                  .then((data) => {
                    this.data=data
                    ...
                    ASSEMBLIES_PARAMETERS1={
                        [app.GridCells.DESCRIPTION_INFO]: ASSEMBLY_DESC1,
                    }
    ...
    it('TC - Add second assembly resource in assemblies module', function () {
        ...
        _common.clear_subContainerFilter(cnt.uuid.ASSEMBLIES)
        _common.create_newRecord(cnt.uuid.ASSEMBLIES)
        _assembliesPage.enterRecord_toCreateAssemblies(cnt.uuid.ASSEMBLIES,ASSEMBLIES_PARAMETERS1)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.minimizeContainer(cnt.uuid.ASSEMBLIES)
        ...
        _common.edit_dropdownCellWithCaret(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.EST_RESOURCE_TYPE_SHORT_KEY,commonLocators.CommonKeys.GRID,CONTAINERS_ASSEMBLY_RESOURCE.RESOURCE_TYPE1)
        _common.clickOn_cellInSubContainer(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.DESCRIPTION_INFO)
        _common.edit_dropdownCellWithInput(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CODE,commonLocators.CommonKeys.GRID,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_ASSEMBLY_RESOURCE.CODE3)
        _common.clickOn_cellInSubContainer(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.QUANTITY_FACTOR_1)
        _common.edit_containerCell(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.QUANTITY_FACTOR_1,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_ASSEMBLY_RESOURCE.QUANTITY_FACTOR)
    ...
    it('TC - Check IsRate and update Rate(Project) in cost code job rates and verify in assembly resources in project', function () {
        ...
        _common.clear_subContainerFilter(cnt.uuid.COSTCODESINPROJECT)
        _common.search_inSubContainer(cnt.uuid.COSTCODESINPROJECT,CONTAINERS_ASSEMBLY_RESOURCE.DESCRIPTION3)
        _common.clickOn_cellHasUniqueValue(cnt.uuid.COSTCODESINPROJECT,app.GridCells.DESCRIPTION,CONTAINERS_ASSEMBLY_RESOURCE.DESCRIPTION3)
        cy.wait(1000)//required wait
        _common.getText_fromCell(cnt.uuid.COSTCODESINPROJECT, app.GridCells.RATE,BASE_COST_CODE_RATE).then(($ele1: JQuery<HTMLElement>) => {
            Cypress.env("CostUnitOriginal2", $ele1.text())
            }) 
        ...
        _common.select_rowInContainer(cnt.uuid.COSTCODESJOBRATE)
        _common.edit_containerCell(cnt.uuid.COSTCODESJOBRATE,app.GridCells.RATE,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_ASSEMBLY_RESOURCE.RATE1)
        cy.SAVE()
        ...
        _common.clear_subContainerFilter(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE)
        _common.search_inSubContainer(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE,CONTAINERS_ASSEMBLY_RESOURCE.DESCRIPTION1)
        _common.select_rowInSubContainer(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE)
        ...
        _common.clear_subContainerFilter(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE)
        _common.search_inSubContainer(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE,CONTAINERS_ASSEMBLY_RESOURCE.DESCRIPTION3)
        _common.select_rowInSubContainer(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE)
        
        _common.assert_cellData_insideActiveRow(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE,app.GridCells.COST_UNIT,CONTAINERS_ASSEMBLY_RESOURCE.RATE1)
    ...
    it('TC - Uncheck IsRate and update Rate(Project) in cost code job rates and verify in assembly resources in project', function () {
       ...
       _common.assert_cellData_insideActiveRow(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE,app.GridCells.COST_UNIT,Cypress.env("CostUnitOriginal"))  
        ...
        _common.search_inSubContainer(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE,CONTAINERS_ASSEMBLY_RESOURCE.DESCRIPTION3)
        _common.select_rowInSubContainer(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE)
        _common.assert_cellData_insideActiveRow(cnt.uuid.PROJECT_ASSEMBLY_RESOURCE,app.GridCells.COST_UNIT,Cypress.env("CostUnitOriginal2"))  
...
it('TC - Delete assembly resource in assemblies module and verify it in project assembly resource', function () {
    ...
    _common.clear_subContainerFilter(cnt.uuid.ASSEMBLY_RESOURCE)
    _common.waitForLoaderToDisappear()
    _common.search_inSubContainer(cnt.uuid.ASSEMBLY_RESOURCE,CONTAINERS_ASSEMBLY_RESOURCE.DESCRIPTION2)
    _common.select_rowInSubContainer(cnt.uuid.ASSEMBLY_RESOURCE)
