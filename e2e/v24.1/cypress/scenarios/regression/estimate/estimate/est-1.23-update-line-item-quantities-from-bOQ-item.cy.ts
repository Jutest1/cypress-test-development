...
ALLURE.story('EST- 1.23 | Update line item quantities from BOQ item');

describe('EST- 1.23 | Update line item quantities from BOQ item', () => {

	before(function () {

		cy.fixture('estimate/est-1.23-update-line-item-quantities-from-bOQ-item.json')
		  .then((data) => {
			this.data = data;
	...
	it("TC - Generate boq line item", function () {
        ...
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
			...
			_common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.estqtytelaotfk,CONTAINER_COLUMNS_LINE_ITEM.estqtyrelboqfk,CONTAINER_COLUMNS_LINE_ITEM.quantitytarget,CONTAINER_COLUMNS_LINE_ITEM.wqquantitytarget],cnt.uuid.ESTIMATE_LINEITEMS)
			_common.waitForLoaderToDisappear()
        });
	...
	it('TC - Update quantity under BOQ structure', function () {
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT);
		_common.openTab(app.TabBar.BOQS).then(() => {
			_common.select_tabFromFooter(cnt.uuid.BOQS, app.FooterTab.BOQs, 2);
		...
		_common.enterRecord_inNewRow(cnt.uuid.BOQ_STRUCTURES,app.GridCells.QUANTITY_SMALL,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_BOQ_STRUCTURE.QUANTITY_UPDATED)
	...
	it('TC - Update estimate and verify AQ quantity in line item with BoQ quantity', function () {
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT);
		_common.openTab(app.TabBar.ESTIMATE).then(() => {
			_common.select_tabFromFooter(cnt.uuid.ESTIMATE, app.FooterTab.ESTIMATE, 2);
		...
		_common.search_inSubContainer(cnt.uuid.ESTIMATE_LINEITEMS,BOQ_STRUCTURE_DESC)
		_common.select_rowHasValue(cnt.uuid.ESTIMATE_LINEITEMS,BOQ_STRUCTURE_DESC)
		_common.assert_forNumericValues(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.QUANTITY_TARGET,CONTAINERS_BOQ_STRUCTURE.QUANTITY_UPDATED);
	});
});
