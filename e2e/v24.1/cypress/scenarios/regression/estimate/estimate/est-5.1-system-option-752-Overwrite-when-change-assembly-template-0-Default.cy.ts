...
import { _common, _projectPage, _estimatePage, _package } from "cypress/pages";
...
ALLURE.story("EST- 5.1 | System Option 752 (Overwrite when change assembly template) = 0 (Default)");
describe("EST- 5.1 | System Option 752 (Overwrite when change assembly template) = 0 (Default)", () => {

    before(function () {

        cy.fixture("estimate/est-5.1-system-option-752-Overwrite-when-change-assembly-template-0-Default.json").then((data) => {
            this.data = data
    ...
    it("TC - Precondition : system option in customizing  overwrite when change assembly template = 0", function () {
    ...
    it("TC - Verify line item description gets append and UoM and quantity gets inherit from assigned assembly", function () {
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.DESCRIPTION_INFO, (LINEITEM_DESC + " " + Cypress.env(ASSEMBLY_DESCRIPTION)))
