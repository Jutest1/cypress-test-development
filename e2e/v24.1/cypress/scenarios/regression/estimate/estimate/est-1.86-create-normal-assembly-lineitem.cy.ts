...
let ASSEMBLY_RESOURCE_PARAMETERS_SA:DataCells
...
ALLURE.story("EST- 1.86 | Create  normal assembly line item");

describe("EST- 1.86 | Create  normal assembly line item", () => {
 
    before(function () {
        cy.fixture("estimate/est-1.86-create-normal-assembly-lineitem.json")
          .then((data) => {
            this.data=data
            ...
            ASSEMBLY_RESOURCE_PARAMETERS_SA={
                [app.GridCells.EST_RESOURCE_TYPE_SHORT_KEY]:CONTAINERS_ASSEMBLY_CATEGORIES.SHORT_KEY_SA,
                [app.GridCells.CODE]:CONTAINERS_ASSEMBLY_CATEGORIES.EXISTING_ASSEMBLIES_SA,
                [app.GridCells.QUANTITY_SMALL]:CONTAINERS_ASSEMBLY_CATEGORIES.QUANTITY_3
            }
    ...
    it("TC - Create assembly category", function () {
        ...
        _common.clear_subContainerFilter(cnt.uuid.ASSEMBLY_CATEGORIES)
        _common.search_inSubContainer(cnt.uuid.ASSEMBLY_CATEGORIES,CONTAINERS_ASSEMBLY_CATEGORIES.EXISTING_ASSEMBLY_CATEGORY_SA)
        _common.clickOn_activeRowCell(cnt.uuid.ASSEMBLY_CATEGORIES,app.GridCells.MARKER)
        _common.clickOn_activeRowCell(cnt.uuid.ASSEMBLY_CATEGORIES,app.GridCells.EST_ASSEMBLY_TYPE_FK)
        _common.clickOn_activeContainerButton(cnt.uuid.ASSEMBLY_CATEGORIES,btn.IconButtons.ICO_INPUT_DELETE)
        _common.select_activeRowInContainer(cnt.uuid.ASSEMBLY_CATEGORIES)
    ...
    it("TC - Create assembly resources", function () {
        ...
        _common.create_newRecord(cnt.uuid.ASSEMBLY_RESOURCE)
        _common.waitForLoaderToDisappear()
        _assembliesPage.enterRecord_toCreateAssemblyResource(ASSEMBLY_RESOURCE_PARAMETERS_SA)
        _common.waitForLoaderToDisappear()
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.saveCellDataToEnv(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.COST_UNIT,"AS_4_COST_PER_UNIT")
        _common.saveCellDataToEnv(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CODE,"SA_CODE")
    ...
    it("TC - Verify resources gets assigned automatically after assigning assembly template to line item in the resources container of estimate and Verify Cost/unit values of resources with assembly resources", function() {
        ...
        _common.clickOn_cellHasUniqueValue(cnt.uuid.RESOURCES,app.GridCells.CODE,Cypress.env("SA_CODE"))
        _common.clickOn_activeRowCell(cnt.uuid.RESOURCES,app.GridCells.TREE)
        _common.assert_forNumericValues(cnt.uuid.RESOURCES,app.GridCells.COST_UNIT,(parseFloat(Cypress.env("AS_4_COST_PER_UNIT"))).toString())
        _validate.verify_activeRowsCellCheckboxValue(cnt.uuid.RESOURCES,app.GridCells.IS_COST,commonLocators.CommonKeys.CHECK)
