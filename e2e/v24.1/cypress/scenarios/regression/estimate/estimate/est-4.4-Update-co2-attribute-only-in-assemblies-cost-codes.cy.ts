...
import { _common, _assembliesPage, _validate, _estimatePage } from "cypress/pages";
...
ALLURE.story("EST- 4.4 | Update CO2/kg attribute only in assemblies cost codes")

describe("EST- 4.4 | Update CO2/kg attribute only in assemblies cost codes", () => {

    before(function () {
        cy.fixture("estimate/est-4.4-Update-co2-attribute-only-in-assemblies-cost-codes.json").then((data) => {
            this.data = data
    ...
    it('TC - Verify cost code updated after recalculate assembly', function () {
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.ASSEMBLIES).then(() => {
            _common.select_tabFromFooter(cnt.uuid.ASSEMBLY_CATEGORIES, app.FooterTab.ASSEMBLYCATEGORY, 0);
        });
