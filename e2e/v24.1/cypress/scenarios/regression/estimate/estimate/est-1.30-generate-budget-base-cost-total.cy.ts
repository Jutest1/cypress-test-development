import { _common, _estimatePage, _validate, _mainView, _boqPage, _modalView, _bidPage } from 'cypress/pages';
...
describe('EST- 1.30 | Generate Budget (Base Cost Total)', () => {
	...
	before(function () {
		cy.fixture('estimate/est-1.30-generate-budget-base-cost-total.json')
		  .then((data) => {
			this.data = data;
	...
	it('TC - Verify Budget gets calculated based on Base Cost Total and x factor', function () {
        ...
        _common.select_rowInContainer(cnt.uuid.ESTIMATE_LINEITEMS)
        _common.waitForLoaderToDisappear()
		_validate.verify_Budget(commonLocators.CommonKeys.BASE_COST_TOTAL, MODAL_GENERATE_BUDGET.X_FACTOR, cnt.uuid.ESTIMATE_LINEITEMS);
	});
});
