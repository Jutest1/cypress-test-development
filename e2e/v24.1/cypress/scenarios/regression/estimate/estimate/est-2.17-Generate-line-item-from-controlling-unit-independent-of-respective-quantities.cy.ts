...
ALLURE.story("EST- 2.17 | Generate line item having quantity independent of controlling unit quantity");
describe("EST- 2.17 | Generate line item having quantity independent of controlling unit quantity", () => {
  before(function () {
    cy.fixture("estimate/est-2.17-Generate-line-item-from-quantity-independent-of-controlling-unit-quantity.json")
      .then((data) => {
        this.data=data
  ...
  it("TC - Verify the quantity in the line item container", function () {
    ...
    _common.assert_cellData_insideActiveRow(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.EST_QTY_REL_GT_UFK,commonLocators.CommonKeys.NO_RELATION)
    _common.assert_forNumericValues_notEqualCondition(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.QUANTITY_TARGET, CONTAINERS_CONTROLLING_UNIT.QUANTITY)
    _common.assert_forNumericValues_notEqualCondition(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.WQ_QUANTITY_TARGET, CONTAINERS_CONTROLLING_UNIT.QUANTITY)
  });
})