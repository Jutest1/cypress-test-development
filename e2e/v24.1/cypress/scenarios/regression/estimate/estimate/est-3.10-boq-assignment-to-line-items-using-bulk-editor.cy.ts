...
ALLURE.story("EST- 3.10 | BOQ assignment to line items using bulk editor");
describe("EST- 3.10 | BOQ assignment to line items using bulk editor", () => {

  before(function () {
    cy.fixture("estimate/est-3.10-boq-assignment-to-line-items-using-bulk-editor.json")
      .then((data) => {
        this.data = data;
  ...
  it("TC - Create new line item record", function () {
    ...
    _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
        ...
        _common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.boqitemfk],cnt.uuid.ESTIMATE_LINEITEMS)
    });
    _common.waitForLoaderToDisappear()
  ...
  it("TC - Verify bulk editor records.", function () {
    _common.select_allContainerData(cnt.uuid.ESTIMATE_LINEITEMS);
    _validate.verify_bulkEditRecords(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.BOQ_ITEM_FK, CONTAINERS_LINE_ITEM.BOQ_ITEM_REF);
  });
});
