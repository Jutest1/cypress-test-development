...
allure.story("EST- 4.23 | Verify updated CO2 attrubutes for a particular material from material package to estimate")

describe("EST- 4.23 | Verify updated CO2 attrubutes for a particular material from material package to estimate", () => {

    beforeEach(function () {
        cy.fixture("estimate/est-4.23-verify-updated-co2-attrubutes-for-a-particular-material-from-material-package-to-estimate.json").then((data) => {
            this.data = data
        })
    })
    ...
    it('TC - Pre-Conditions : Set status value, and Add CO2 project value to records in material module', function () {
        ...
        _common.search_inSubContainer(cnt.uuid.DATA_TYPES, CONTAINERS_DATA_TYPES.PACKAGE_STATUS)
        _common.select_rowHasValue(cnt.uuid.DATA_TYPES, CONTAINERS_DATA_TYPES.PACKAGE_STATUS)
        _common.openTab(app.TabBar.MASTER_DATA).then(() => {
        ...
        _common.select_rowHasValue(cnt.uuid.DATA_RECORDS, CONTAINERS_DATA_RECORDS.IN_PROGRESS)
        _common.set_cellCheckboxValue(cnt.uuid.DATA_RECORDS, app.GridCells.IS_ESTIMATE, commonLocators.CommonKeys.CHECK)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
    ...
    it('TC - Create a Material Package, update items CO2 attribute, and Update Estimate using wizard.', function () {
    ...
    it('TC - Verify the CO2 of added material in Estimate..', function () {
        ...
        _common.assert_forNumericValues(cnt.uuid.RESOURCES, app.GridCells.CO2_PROJECT, CONTAINER_MATERIAL.CO2_PROJECT_VALUE)
    })
    after(() => {
        cy.LOGOUT();
    });
