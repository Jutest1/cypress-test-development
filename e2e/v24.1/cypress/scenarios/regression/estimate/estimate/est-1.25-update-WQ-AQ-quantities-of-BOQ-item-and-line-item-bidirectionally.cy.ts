...
let UPDATE_ESTIMATE_TO_WIZARD_PARAMETERS:DataCells
let MODAL_UPDATE_ESTIMATE_TO_WIZARD

ALLURE.epic('ESTIMATE');
ALLURE.feature('Estimate');
ALLURE.story('EST- 1.25 | Update WQ AQ quantities of BOQ item & line item bidirectionally');

describe('EST- 1.25 | Update WQ AQ quantities of BOQ item & line item bidirectionally', () => {

	before(function () {

		cy.fixture('estimate/est-1.25-update-WQ-AQ-quantities-of-BOQ-item-&-line-item-bidirectionally.json')
		  .then((data) => {
			this.data = data;
			...
			MODAL_UPDATE_ESTIMATE_TO_WIZARD=this.data.MODAL.UPDATE_ESTIMATE_TO_WIZARD

			UPDATE_ESTIMATE_TO_WIZARD_PARAMETERS={
				[commonLocators.CommonKeys.CHECKBOX]:MODAL_UPDATE_ESTIMATE_TO_WIZARD
			}
		  }).then(()=>{
	...
	it('TC - Update estimate and verify update to boq', function () {
	...
	it('TC - Verify updated AQ quantity of BoQ structure', function () {
		...
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD)
		_common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.UPDATE_ESTIMATE);
		_estimatePage.update_estimate_fromWizard(UPDATE_ESTIMATE_TO_WIZARD_PARAMETERS);
		_common.waitForLoaderToDisappear()
		cy.SAVE()
		_common.waitForLoaderToDisappear()
		cy.REFRESH_CONTAINER()
