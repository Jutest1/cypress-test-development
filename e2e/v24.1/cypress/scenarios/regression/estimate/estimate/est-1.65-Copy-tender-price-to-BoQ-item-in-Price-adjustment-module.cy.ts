...
const WQ_ADJUSTMENT_PRICE = 'AQ_ADJUSTMENT_PRICE'
...
describe('EST- 1.65 | Copy tender price to BoQ item in Price adjustment module', () => {
  before(function () {
    ...
    cy.fixture('estimate/est-1.65-Copy-tender-price-to-BoQ-item-in-Price-adjustment-module.json').then((data) => {
      this.data = data;
  ...
  it('TC - Update adjustment price WQ', function () {
    _common.openTab(app.TabBar.ESTIMATEBYBOQ).then(() => {
      ...
      _common.setup_gridLayout(cnt.uuid.PRICEADJUSTMENT, CONTAINER_COLUMNS_PRICE_ADJUST);
			_common.set_columnAtTop([CONTAINER_COLUMNS_PRICE_ADJUST.wqadjustmentprice],cnt.uuid.PRICEADJUSTMENT)
    })
    _common.waitForLoaderToDisappear()
    _estimatePage.updateRecordTo_AdjustPriceWQ(CONTAINERS_PRICE_ADJUST.WQ_ADJUSTMENT_PRICE, BOQ_STRUCT_DESC)
    _common.waitForLoaderToDisappear()
    cy.SAVE();
    _common.saveCellDataToEnv(cnt.uuid.PRICEADJUSTMENT, app.GridCells.WQ_ADJUSTMENT_PRICE, WQ_ADJUSTMENT_PRICE)
  })

  it("TC - Copy tender price and verify it with boq item", function () {
    _estimatePage.copyTenderPrice(commonLocators.CommonModalElements.MODAL_ROW_CLASS, 'option2');
    _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
    _common.waitForLoaderToDisappear()
    _common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT)
    _common.waitForLoaderToDisappear()
    ...
    _common.waitForLoaderToDisappear()
    _common.getText_fromCell(cnt.uuid.BOQ_STRUCTURES, app.GridCells.FINAL_PRICE_SMALL).then(($ele1: JQuery<HTMLElement>) => {
      var finalPrice = parseFloat($ele1.text().replace(",", "")).toFixed(2)
      expect(finalPrice).to.equals(Cypress.env(WQ_ADJUSTMENT_PRICE));
    })
  })
