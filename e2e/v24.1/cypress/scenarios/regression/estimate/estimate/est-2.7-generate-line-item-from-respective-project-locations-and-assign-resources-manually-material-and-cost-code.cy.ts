...
describe('EST- 2.7 | Generate Line item from respective Project locations and assign resources manually (material and cost code)', () => {
	...
	before(function () {
		cy.fixture('estimate/est-2.7-generate-line-item-from-respective-project-locations-and-assign-resources-manually-material-and-cost-code.json')
		  .then((data) => {
			this.data = data;
	...
	it('TC - Create new location', function () {
		...
		_common.enterRecord_inNewRow(cnt.uuid.PROJECT_LOCATION, app.GridCells.SORTING, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_LOCATION.SORTING);
		...
		_common.enterRecord_inNewRow(cnt.uuid.PROJECT_LOCATION, app.GridCells.SORTING, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_LOCATION.SORTING_1);
		_common.select_activeRowInContainer(cnt.uuid.PROJECT_LOCATION)
		_common.waitForLoaderToDisappear()
		cy.SAVE()
		_common.waitForLoaderToDisappear()
