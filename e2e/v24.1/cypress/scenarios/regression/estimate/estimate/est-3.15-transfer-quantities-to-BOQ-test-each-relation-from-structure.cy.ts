...
ALLURE.story('EST- 3.15 | Transfer quantities to BOQ relation - from structure');

describe('EST- 3.15 | Transfer quantities to BOQ relation - from structure', () => {
	

	before(function () {
		cy.fixture('estimate/est-3.15-Transfer-quantities-to-BOQ-test-each-relation-from-structure.json')
		  .then((data) => {
			this.data = data;
	...
	it("TC - Generate boq line item", function () {
		...
		_common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
			...
			_common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.quantitytarget,CONTAINER_COLUMNS_LINE_ITEM.estqtyrelboqfk,CONTAINER_COLUMNS_LINE_ITEM.costtotal],cnt.uuid.ESTIMATE_LINEITEMS)
		});
	...
	it('TC - Verify line item aq quantity with boq quantity and edit line item', function () {
		...
		_common.edit_dropdownCellWithCaret(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.EST_QTY_REL_BOQ_FK,commonLocators.CommonKeys.SPAN,commonLocators.CommonKeys.FROM_STRUCTURE);
	...
	it('TC - Edit boq structure aq quantity', function () {
		_common.waitForLoaderToDisappear()
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART);
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT);
		_common.waitForLoaderToDisappear()
		...
		_common.enterRecord_inNewRow(cnt.uuid.BOQ_STRUCTURES,app.GridCells.QUANTITY_ADJ,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_BOQ_STRUCTURE.QUANTITY_1)
	...
	it('TC - Verify updated aq quantity of BoQ structure - from structure', function () {
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT);
		_common.openTab(app.TabBar.ESTIMATE).then(() => {
			_common.select_tabFromFooter(cnt.uuid.ESTIMATE, app.FooterTab.ESTIMATE, 2);
		...
		_common.search_inSubContainer(cnt.uuid.ESTIMATE_LINEITEMS,BOQ_STRUCTURE_DESC)
		_common.select_rowHasValue(cnt.uuid.ESTIMATE_LINEITEMS,BOQ_STRUCTURE_DESC)
		_common.assert_forNumericValues(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.QUANTITY_TARGET,CONTAINERS_BOQ_STRUCTURE.QUANTITY_1)
	});
});
