...
const WQ_ADJUSTMENT_PRICE = 'WQ_ADJUSTMENT_PRICE'
...
describe("EST- 1.64 | Copy tender price to line item in price adjustment module", () => {
  before(function () {
    cy.fixture(
      "estimate/est-1.64-Copy-tender-price-to-Line-item-in-Price-adjustment-module.json").then((data) => {
        this.data = data;
  ...
  it("TC - Copy tender price and verify it with line item", function () {
    ...
    cy.wait(2000).then(() => { //required wait to load page
      _common.assert_cellData_insideActiveRow(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.GRAND_TOTAL, Cypress.env(WQ_ADJUSTMENT_PRICE))
    })
  });
