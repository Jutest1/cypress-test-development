...
ALLURE.story("EST- 2.10 | Generate line item from location independent of respective quantities");
describe("EST- 2.10 | Generate line item from location independent of respective quantities", () => {
  
  	before(function () {
    cy.fixture("estimate/est-2.10-generating-line-items-from-location-independent-of-respective-quantities.json")
      .then((data) => {
        this.data=data
	...
	it("TC - Verify the quantity in the line item container", function () {
		...
		_common.assert_forNumericValues_notEqualCondition(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.QUANTITY_SMALL, CONTAINERS_LOCATION.QUANTITY_FACTOR);
	});
});