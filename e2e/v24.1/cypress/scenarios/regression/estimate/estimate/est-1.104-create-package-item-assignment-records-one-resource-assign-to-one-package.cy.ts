...
allure.story("EST- 1.104 | Create package item assignment records: One resource assign to one package ");

describe("EST- 1.104 | Create package item assignment records: One resource assign to one package", () => {
    beforeEach(function () {
        cy.fixture("estimate/est-1.104-create-package-item-assignment-records-one-resource-assign-to-one-package.json").then((data) => {
            this.data = data;
        });
    });
    ...
    before(() => {
        ...
        cy.fixture("estimate/est-1.104-create-package-item-assignment-records-one-resource-assign-to-one-package.json").then(function (data) {
            this.data = data;
    ...
    it("TC- Create material package manually and add resource to it", function () {
        ...
        _common.openTab(app.TabBar.PACKAGE).then(() => {
            ...
            _common.waitForLoaderToDisappear()
            //  _common.set_columnAtTop([CONTAINER_COLUMNS_PACKAGE_ITEM.mdcmaterialfk,CONTAINER_COLUMNS_PACKAGE_ITEM.quantity, CONTAINER_COLUMNS_PACKAGE_ITEM.co2project], cnt.uuid.PACKAGEITEMS)
            _common.waitForLoaderToDisappear()
