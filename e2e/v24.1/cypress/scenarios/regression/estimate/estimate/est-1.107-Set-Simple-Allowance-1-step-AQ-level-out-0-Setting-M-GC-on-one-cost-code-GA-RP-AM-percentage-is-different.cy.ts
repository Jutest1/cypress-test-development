...
import { _common, _boqPage, _estimatePage, _projectPage } from "cypress/pages";
...
ALLURE.story("EST- 1.107 | Set Simple Allowance: 1-step, AQ, level out=  0; Setting  M(GC)[%] on one cost code; GA RP AM percentage is different");

describe("EST- 1.107 | Set Simple Allowance: 1-step, AQ, level out=  0; Setting  M(GC)[%] on one cost code; GA RP AM percentage is different", () => {

    before(function () {

        cy.fixture("estimate/est-1.107-Set-Simple-Allowance-1-step-AQ-level-out=0-Setting-M(GC)[%]-on-one-cost-code-GA-RP-AM-percentage-is-different.json")
            .then((data) => {
                this.data = data;
                ...
                ALLOWANCE_MARKUP_PARAMETERS3 = {
                    ...
                    [app.GridCells.AM_PERC]: CONTAINERS_ALLOWANCE_MARKUP.AM[1],
                }
                ALLOWANCE_MARKUP_PARAMETERS4 = {
                    ...
                    [app.GridCells.RP_PERC]: CONTAINERS_ALLOWANCE_MARKUP.RP[2],
                    [app.GridCells.AM_PERC]: CONTAINERS_ALLOWANCE_MARKUP.AM[2],
                }
    ...
    it('TC - Assign material and costcode resources to the first line item', function () {
        ...
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
            ...
            _common.set_columnAtTop([CONTAINER_COLUMNS_RESOURCES.estresourcetypeshortkey, CONTAINER_COLUMNS_RESOURCES.code], cnt.uuid.RESOURCES)
        });
    ...
    it('TC - Verify create standerd allowance', function () {
        _common.clickOn_cellHasUniqueValue(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.DESCRIPTION_INFO, BOQ_ITEM_DESCRIPTION_1)
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
    ...
    it('TC - Add Allowance/Unit and Grand Cost/unit', function () {
        ...
        _common.enterRecord_inNewRow(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.ADVANCED_ALL_UNIT_ITEM, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_LINE_ITEM.ADV_ALLOWANCE_PER_UNIT_ITEMM);
        cy.SAVE();
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.ESTIMATE_LINEITEMS, BOQ_ITEM_DESCRIPTION_2)
    ...
    it('TC - Verify Totals in Totals container', function () {
        _common.clickOn_cellHasUniqueValue(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.DESCRIPTION_INFO, BOQ_DESCRIPTION_2)
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
