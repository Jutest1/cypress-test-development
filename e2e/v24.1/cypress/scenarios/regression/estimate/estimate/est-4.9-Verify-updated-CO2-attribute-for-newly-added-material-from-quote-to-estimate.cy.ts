...
ALLURE.story("EST- 4.9 | Verify updated CO2 attributes for a newly added material from quote to estimate")
describe("EST- 4.9 | Verify updated CO2 attributes for a newly added material from quote to estimate", () => {
   
    before(function () {
       
        cy.fixture("estimate/est-4.9-Verify-updated-CO2-attribute-for-newly-added-material-from-quote-to-estimate.json")
          .then((data) => {
            this.data=data
            ...
            RESOURCE_PARAMETERS_4 = {
				...
				[app.GridCells.CODE]: CONTAINERS_MATERIAL_RECORDS.MATERIAL_RECORD_DESCRIPTION[0]
			};
    ...
    it("TC - Create quote and add new item record", function () {
        ...
        _common.edit_dropdownCellWithInput(cnt.uuid.QUOTES_ITEMS,app.GridCells.MDC_MATERIAL_FK,commonLocators.CommonKeys.GRID,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_MATERIAL_RECORDS.MATERIAL_RECORD_DESCRIPTION[1])
        _common.enterRecord_inNewRow(cnt.uuid.QUOTES_ITEMS,app.GridCells.QUANTITY_SMALL,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_MATERIAL_RECORDS.QUANTITY)
        ...
        cy.SAVE()
        _common.waitForLoaderToDisappear()    
        _common.search_inSubContainer(cnt.uuid.QUOTES_ITEMS,CONTAINERS_MATERIAL_RECORDS.MATERIAL_RECORD_DESCRIPTION[1])
        _common.waitForLoaderToDisappear()    
        _common.select_rowHasValue(cnt.uuid.QUOTES_ITEMS,CONTAINERS_MATERIAL_RECORDS.MATERIAL_RECORD_DESCRIPTION[1])
        _common.waitForLoaderToDisappear()    
        ...
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        cy.wait(1000) // This wait is required as container took time to load
        _common.clickOn_modalFooterButton(btn.ButtonText.OK)
        _common.waitForLoaderToDisappear()  
    })
   
    it('TC - Verify new line item record created and CO2 project Total for the same is updated', function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT);
		...
		_common.clear_subContainerFilter(cnt.uuid.ESTIMATE_LINEITEMS)
		_common.search_inSubContainer(cnt.uuid.ESTIMATE_LINEITEMS,commonLocators.CommonKeys.MATERIAL)
        ...
        let lineItemQuantity:any=parseFloat(CONTAINERS_MATERIAL_RECORDS.QUANTITY).toFixed(2)
        let co2Project_Total=co2ProjectTotal*lineItemQuantity
        _common.assert_forNumericValues(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.CO2_PROJECT_TOTAL,co2Project_Total.toString())     
