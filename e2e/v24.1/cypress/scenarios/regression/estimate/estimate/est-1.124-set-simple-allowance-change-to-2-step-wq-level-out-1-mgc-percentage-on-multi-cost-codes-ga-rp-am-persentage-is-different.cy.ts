...
ALLURE.story("EST- 1.124 | Set Simple Allowance, Change to  2-step, WQ, level out=  1;Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different");

describe("EST- 1.124 | Set Simple Allowance, Change to  2-step, WQ, level out=  1;Setting  M(GC)[%] on multi costcode; GA RP AM percentage is different", () => {

    before(function () {

        cy.fixture("estimate/est-1.124-set-simple-allowance-change-to-2-step-wq-level-out-1-mgc-percentage-on-multi-cost-codes-ga-rp-am-persentage-is-different.json")
            .then((data) => {
                this.data = data;
    ...
    it('TC - Verify create standard allowance', function () {
        ...
        _common.select_activeRowInContainer(cnt.uuid.STANDARD_ALLOWANCES)
        _common.edit_containerCell(cnt.uuid.STANDARD_ALLOWANCES, app.GridCells.MARKUP_GA, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_STANDARD_ALLOWANCE.GA[0])
        _common.edit_containerCell(cnt.uuid.STANDARD_ALLOWANCES, app.GridCells.MARKUP_RP, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_STANDARD_ALLOWANCE.RP[0])
        _common.edit_containerCell(cnt.uuid.STANDARD_ALLOWANCES, app.GridCells.MARKUP_AM, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_STANDARD_ALLOWANCE.AM[0])
        _common.select_activeRowInContainer(cnt.uuid.STANDARD_ALLOWANCES)
        cy.SAVE();
        _common.waitForLoaderToDisappear()
    ...
    it('TC - Verify GA, RP, AM percentage in Allowance Markup', function () {
        ...
        _common.select_activeRowInContainer(cnt.uuid.ALLOWANCEMARKUP)
        _common.edit_containerCell(cnt.uuid.ALLOWANCEMARKUP, app.GridCells.DEF_M_PERC, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_ALLOWANCE_MARKUP.DEF_M[0])
        _common.select_activeRowInContainer(cnt.uuid.ALLOWANCEMARKUP)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.ALLOWANCEMARKUP, CONTAINERS_ALLOWANCE_MARKUP.COST_CODE[1])
    ...
    it('TC - Verify Totals in Totals container', function () {
        ...
        _common.saveCellDataToEnv(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.COST_TOTAL, "LiCostTotal")
        _common.perform_additionOfCellData(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.COST_TOTAL)
        cy.log("Line Item Cost Total" + Cypress.env("AdditionOfColumnValues"))
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
            _common.select_tabFromFooter(cnt.uuid.STANDARD_ALLOWANCES, app.FooterTab.STANDARS_ALLOWANCES, 2);
            _common.setup_gridLayout(cnt.uuid.STANDARD_ALLOWANCES, CONTAINER_COLUMNS_STANDARD_ALLOWANCE);
        });
        ...
        cy.wait(1000).then(() => {
            ...
            _common.waitForLoaderToDisappear()
            var GA_VALUE = (parseFloat(Cypress.env("AdditionOfColumnValues")) * ((parseFloat(CONTAINERS_ALLOWANCE_MARKUP.GA[0])) / 100)).toFixed(2).toString()
            _common.assert_forNumericValues(cnt.uuid.ESTIMATETOTALS, app.GridCells.TOTAL, GA_VALUE)
            _common.clickOn_cellHasUniqueValue(cnt.uuid.ESTIMATETOTALS, app.GridCells.DESCRIPTION, CONTAINERS_TOTALS.TOTALS_TYPE[1])
            _common.select_activeRowInContainer(cnt.uuid.ESTIMATETOTALS)
            _common.waitForLoaderToDisappear()
            var RP_VALUE = (parseFloat(Cypress.env("AdditionOfColumnValues")) * ((parseFloat(CONTAINERS_ALLOWANCE_MARKUP.RP[0])) / 100)).toFixed(2).toString()
            _common.assert_forNumericValues(cnt.uuid.ESTIMATETOTALS, app.GridCells.TOTAL, RP_VALUE)
            _common.clickOn_cellHasUniqueValue(cnt.uuid.ESTIMATETOTALS, app.GridCells.DESCRIPTION, CONTAINERS_TOTALS.TOTALS_TYPE[2])
            _common.select_activeRowInContainer(cnt.uuid.ESTIMATETOTALS)
            _common.waitForLoaderToDisappear()
            _common.saveCellDataToEnv(cnt.uuid.ESTIMATETOTALS, app.GridCells.TOTAL, "Calculated_GC")
            _common.clickOn_cellHasUniqueValue(cnt.uuid.ESTIMATETOTALS, app.GridCells.DESCRIPTION, CONTAINERS_TOTALS.TOTALS_TYPE[3])
            _common.select_activeRowInContainer(cnt.uuid.ESTIMATETOTALS)
            _common.waitForLoaderToDisappear()
            var AM_VALUE = (parseFloat(Cypress.env("AdditionOfColumnValues")) * ((parseFloat(CONTAINERS_ALLOWANCE_MARKUP.AM[0])) / 100)).toFixed(2).toString()
            _common.assert_forNumericValues(cnt.uuid.ESTIMATETOTALS, app.GridCells.TOTAL, AM_VALUE)
        })
