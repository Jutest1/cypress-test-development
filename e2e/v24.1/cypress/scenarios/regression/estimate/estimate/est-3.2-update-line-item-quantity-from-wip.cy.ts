...
ALLURE.story('EST- 3.2 | Update Line item quantities From WIP');
describe('EST- 3.2 | Update Line item quantities From WIP', () => {


	before(function () {
		cy.fixture('estimate/est-3.2-update-line-item-quantity-from-wip.json')
		  .then((data) => {
				this.data = data;
	...
	it('TC - Verify update BoQ quantity in WIP module', function () {
		...
		_wipPage.updateQuantity_inWIPBoqStructure(CONTAINERS_LINE_ITEM.WIP_QUANTITY);
		_common.openTab(app.TabBar.WIPBOQ).then(() => {
			_common.select_tabFromFooter(cnt.uuid.WIPBOQ, app.FooterTab.BOQs, 0);
		});
		_common.clear_subContainerFilter(cnt.uuid.WIPBOQ)
