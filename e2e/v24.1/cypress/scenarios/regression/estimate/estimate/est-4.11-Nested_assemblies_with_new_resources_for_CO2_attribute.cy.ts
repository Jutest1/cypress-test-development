...
describe("EST- 4.11 | Nested assemblies for new added resource for CO2/kg attribute", () => {
    ...
    before(function () {
        ...
        cy.fixture("estimate/est-4.11-Nested_assemblies_for_new_added_resource_for_CO2_attribute.json")
          .then((data) => {
            this.data=data
    ...
    it("TC - Create assembly resources", function () {
        ...
        cy.wait(1000) // Added this wait as script is getting failed
    ...
    it('TC - Update co2 project attribute for assembly resource of main assembly', function () {
        ...
        _common.clickOn_cellHasUniqueValue(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CODE,CONTAINERS_COST_CODES.CODE[0])
        _common.enterRecord_inNewRow(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CO2_PROJECT,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_COST_CODES.CO2_PROJECT[1])
        _common.select_activeRowInContainer(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.DESCRIPTION_INFO)
        _common.waitForLoaderToDisappear()
        cy.SAVE()
        _common.waitForLoaderToDisappear()

        _common.clickOn_cellHasUniqueValue(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CODE,CONTAINERS_COST_CODES.CODE[1])
        _common.enterRecord_inNewRow(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CO2_PROJECT,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_COST_CODES.CO2_PROJECT[2])
        _common.select_activeRowInContainer(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.DESCRIPTION_INFO)
        _common.waitForLoaderToDisappear()
        cy.SAVE()
        _common.waitForLoaderToDisappear()

       _common.clickOn_cellHasUniqueValue(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.DESCRIPTION_INFO,CONTAINERS_MATERIAL_RECORDS.MATERIAL_RECORD_DESCRIPTION)
        _common.enterRecord_inNewRow(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.CO2_PROJECT,app.InputFields.INPUT_GROUP_CONTENT,CONTAINERS_COST_CODES.CO2_PROJECT[3])
        _common.select_activeRowInContainer(cnt.uuid.ASSEMBLY_RESOURCE,app.GridCells.DESCRIPTION_INFO)
        _common.waitForLoaderToDisappear()
        cy.SAVE()
        _common.waitForLoaderToDisappear()
