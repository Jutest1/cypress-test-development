...
ALLURE.story("EST- 2.19 | Generate line item from activity independent of respective quantity");
describe("EST- 2.19 | Generate line item from activity independent of respective quantity", () => {

  before(function () {
    cy.fixture("estimate/est-2.19-Generate-line-item-from-activity-independent-of-respective-quantities.json")
      .then((data) => {
        this.data = data
        SCH_PARAMETERS={
  ...
  it("TC - Verify the quantity in the line item container", function () { 
    ...
    _common.assert_cellData_insideActiveRow(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.EST_QTY_REL_ACT_FK,commonLocators.CommonKeys.NO_RELATION)
    _common.assert_forNumericValues_notEqualCondition(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.QUANTITY_TARGET, CONTAINERS_SCHEDULING_ACTIVITY.QUANTITY)
    _common.assert_forNumericValues_notEqualCondition(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.WQ_QUANTITY_TARGET, CONTAINERS_SCHEDULING_ACTIVITY.QUANTITY)
  });
})