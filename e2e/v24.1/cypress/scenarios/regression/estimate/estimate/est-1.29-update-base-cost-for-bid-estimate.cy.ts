...
let MODAL_GENERATE_UPDATE_BASE_COST_1
let GENERATE_UPDATE_BASE_COST_1_PARAMETERS: DataCells


ALLURE.epic('ESTIMATE');
ALLURE.feature('Estimate');
ALLURE.story('EST- 1.29 | Update base cost for bid estimate');

describe('EST- 1.29 | Update base cost for bid estimate', () => {
    before(function () {
        cy.fixture('estimate/est-1.29-update-base-cost-for-bid-estimate.json')
            .then((data) => {
                this.data = data;
                ...
                MODAL_GENERATE_UPDATE_BASE_COST_1 = this.data.MODAL.GENERATE_UPDATE_BASE_COST_1
                GENERATE_UPDATE_BASE_COST_1_PARAMETERS = {
                    [commonLocators.CommonLabels.SELECT_ESTIMATE_SCOPE]: MODAL_GENERATE_UPDATE_BASE_COST_1.SELECT_ESTIMATE_SCOPE,
                    [commonLocators.CommonLabels.GENERATE_UPDATE_BASE_COST]: MODAL_GENERATE_UPDATE_BASE_COST_1.GENERATE_UPDATE_BASE_COST,
                    [commonLocators.CommonKeys.INDEX]: MODAL_GENERATE_UPDATE_BASE_COST_1.INDEX,
                    [commonLocators.CommonKeys.RADIO_INDEX]: MODAL_GENERATE_UPDATE_BASE_COST_1.RADIO_INDEX
                }
            }).then(() => {
    ...
    it('TC - Verify Base cost unit equals base cost total/Qty  generated in Line Item container, based on change in Quantity in Resource container ', function () {
        _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
            _common.select_tabFromFooter(cnt.uuid.RESOURCES, app.FooterTab.RESOURCES);
        });
        _common.maximizeContainer(cnt.uuid.RESOURCES)
        ...
        _common.enterRecord_inNewRow(cnt.uuid.RESOURCES, app.GridCells.QUANTITY_SMALL, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_RESOURCE.QUANTITY)
        _common.select_activeRowInContainer(cnt.uuid.RESOURCES)
        _common.waitForLoaderToDisappear()
        cy.SAVE();
        _common.waitForLoaderToDisappear()
        ...
        _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.GENERATE_UPDATE_BASE_COST);
        _common.waitForLoaderToDisappear()
        _estimatePage.generate_updateBaseCost_fromWizard(GENERATE_UPDATE_BASE_COST_1_PARAMETERS)
        _common.waitForLoaderToDisappear()
    });
    it('TC - Verify updated Base cost/unit and Base cost total of line items', function () {
        _common.waitForLoaderToDisappear()
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD);
        _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.GENERATE_UPDATE_BASE_COST);
        _common.waitForLoaderToDisappear()
