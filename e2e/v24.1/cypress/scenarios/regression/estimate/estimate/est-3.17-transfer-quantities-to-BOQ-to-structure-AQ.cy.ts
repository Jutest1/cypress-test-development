...
describe("EST- 3.17 | Transfer quantities to BOQ - to structure AQ", () => {
  ...
  before(function () {
    ...
    cy.fixture("estimate/est-3.17-transfer-quantities-to-BOQ-to-structure-AQ.json")
      .then((data) => {
        this.data = data;
	...
	it('TC - Update BoQ quantities from wizard', function () {
		...
		_common.select_activeRowInContainer(cnt.uuid.ESTIMATE_LINEITEMS)
		_common.waitForLoaderToDisappear()
		_common.edit_dropdownCellWithCaret(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.EST_QTY_REL_BOQ_FK,commonLocators.CommonKeys.SPAN,commonLocators.CommonKeys.TO_STRUCTURE_AQ);
		_common.select_activeRowInContainer(cnt.uuid.ESTIMATE_LINEITEMS)
		_common.waitForLoaderToDisappear()
		cy.SAVE()
		_common.waitForLoaderToDisappear()
