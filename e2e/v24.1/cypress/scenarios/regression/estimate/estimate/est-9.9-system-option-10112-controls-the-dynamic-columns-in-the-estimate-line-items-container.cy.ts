...
import { _common, _estimatePage, _modalView, _mainView, _schedulePage, _validate } from "cypress/pages";
...
ALLURE.story("EST-9.9 | System option 10112 controls the dynamic columns in the estimate line items container");
describe("EST-9.9 | System option 10112 controls the dynamic columns in the estimate line items container", () => {
    before(function () {
      cy.fixture("estimate/est-9.9-system-option-10112-controls-the-dynamic-columns-in-the-estimate-line-items-container.json").then((data) => {
        this.data = data;
  ...
  cy.preLoading(Cypress.env('adminUserName'), Cypress.env('adminPassword'), Cypress.env('parentCompanyName'), Cypress.env('childCompanyName'));
    ...
    it("TC - Precondition : system option in customizing  overwrite when change assembly template = 1", function () {
        ...
        _common.search_inSubContainer(cnt.uuid.DATA_RECORDS, CONTAINERS_DATA_RECORDS.SHOW_DYNAMIC_COLUMNS_FROM_LINE_ITEM_QUANTITY_CONTAINER_IN_LINE_ITEM_CONTAINER)
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.DATA_RECORDS, CONTAINERS_DATA_RECORDS.SHOW_DYNAMIC_COLUMNS_FROM_LINE_ITEM_QUANTITY_CONTAINER_IN_LINE_ITEM_CONTAINER)
  ...
  it("TC - Update Line Item Quantities", function () {
      ...
      _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
        _common.setDefaultView(app.TabBar.ESTIMATELINEITEM)
        _common.waitForLoaderToDisappear()
        _common.select_tabFromFooter(cnt.uuid.LINE_ITEM_QUANTITIES, app.FooterTab.LINE_ITEM_QUANTITY, 3);
  ...
  it("TC - Verify the columns (iq and aq) from the Line Item Quantity Container should be added in the line item container", function () {
    _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
      _common.setDefaultView(app.TabBar.ESTIMATELINEITEM)
      _common.waitForLoaderToDisappear()
      _common.select_tabFromFooter(cnt.uuid.ESTIMATE_LINEITEMS, app.FooterTab.LINE_ITEMS, 0);
  ...
  _validate.verify_isCellPresent(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.IQ)
  _validate.verify_isCellPresent(cnt.uuid.ESTIMATE_LINEITEMS, app.GridCells.AQ)
