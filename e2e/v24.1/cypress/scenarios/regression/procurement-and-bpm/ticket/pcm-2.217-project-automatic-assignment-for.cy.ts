...
ALLURE.story("PCM- 2.217 | Project automatic assignment for ");
describe("PCM- 2.217 | Project automatic assignment for ", () => {
        
    before(function () {
        cy.fixture("pcm/pcm-2.217-project-automatic-assignment-for.json")
          .then((data) => {
            this.data = data;
    ...
    it("TC - Unpin the created project", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART);
        _common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.PROJECT); 
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_SEARCH).delete_pinnedItem()
        _common.search_fromSidebar(commonLocators.CommonKeys.STANDARD," ").delete_pinnedItem();
        cy.SAVE();
        _common.waitForLoaderToDisappear()
    })
    ...
    it("TC - If 'Procurement Type' is 'Requisition', click place order button, in dialog,each fields is working and filter is ok", function () {
        ...
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        _common.edit_dropDownWithInput_fromModal(commonLocators.CommonLabels.PROJECT,PROJECT_NO,commonLocators.CommonKeys.GRID)
    ...
    it("TC - Assertion 2- Requisition and Verify project", function () {
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.REQUISITIONS,app.GridCells.PROJECT_FK,PROJECT_NO)
    })
});
