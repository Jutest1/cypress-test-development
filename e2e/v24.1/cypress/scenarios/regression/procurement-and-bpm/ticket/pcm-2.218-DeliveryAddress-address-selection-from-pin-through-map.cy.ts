...
const PROJECT_NO="37" + Cypress._.random(0, 999);
...
let PROJECTS_1_PARAMETERS:DataCells

let ADDRESS_PARAMETER:DataCells
let CONTAINER_COLUMNS_PROJECT

ALLURE.epic("PROCUREMENT AND BPM");
ALLURE.feature("Ticket System");

ALLURE.story("PCM- 2.218 | Delivery Address - Address selection from pin through map");
describe("PCM- 2.218 | Delivery Address - Address selection from pin through map", () => {
        
    before(function () {
        cy.fixture("procurement-and-bpm/pcm-2.218-DeliveryAddress-address-selection-from-pin-through-map.json")
          .then((data) => {
            this.data = data;
    ...
    it("TC - Create material procurement structure", function () {
        ...
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        cy.wait(2000) // added this wait as script  is getting failed
    ...
    it("TC - Assertion 1 - The delivery address should be null", function () {
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.MAIN).then(() => {
            _common.select_tabFromFooter(cnt.uuid.REQUISITIONS, app.FooterTab.REQUISITION)
            _common.setup_gridLayout(cnt.uuid.REQUISITIONS, CONTAINER_COLUMNS_REQUISITION)
            _common.set_columnAtTop([CONTAINER_COLUMNS_REQUISITION.addressentity],cnt.uuid.REQUISITIONS)
            _common.waitForLoaderToDisappear()
        })
        _common.waitForLoaderToDisappear()
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_SEARCH).delete_pinnedItem()
        ...
        _common.clickOn_activeRowCell(cnt.uuid.REQUISITIONS,app.GridCells.ADDRESS_ENTITY)
        _common.verify_gridCellHasNoText(cnt.uuid.REQUISITIONS,app.GridCells.ADDRESS_ENTITY)
    });
    ...
    it("TC - Create New Project And Add Address", function () {
        ...
        _common.create_newRecord(cnt.uuid.PROJECTS);
        _common.waitForLoaderToDisappear()
        _projectPage.enterRecord_toCreateProject(PROJECTS_1_PARAMETERS);
        _common.waitForLoaderToDisappear()
        cy.SAVE();          
        _common.waitForLoaderToDisappear()
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_SEARCH).delete_pinnedItem()
        ...
        _common.select_rowHasValue(cnt.uuid.PROJECTS,PROJECT_NO_1)
        _common.waitForLoaderToDisappear()
        ...
        _common.saveCellDataToEnv(cnt.uuid.PROJECTS, app.GridCells.ADDRESS_FK, "PROJECT_ADDRESS")
    });
    ...
    it("TC - Assertion 2 - Go to Cart and Create Requisition and Verify Address", function () {
        _common.openTab(app.TabBar.CART).then(() => {
            _common.select_tabFromFooter(cnt.uuid.CART_ITEMS, app.FooterTab.CART_ITEMS)
        })
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        _common.edit_dropDownWithInput_fromModal(commonLocators.CommonLabels.PROJECT,PROJECT_NO_1,commonLocators.CommonKeys.GRID)
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.REQUISITIONS,app.GridCells.PROJECT_FK,PROJECT_NO_1)
        _common.maximizeContainer(cnt.uuid.REQUISITIONS)
        _common.assert_cellData_insideActiveRow(cnt.uuid.REQUISITIONS, app.GridCells.ADDRESS_ENTITY, Cypress.env("PROJECT_ADDRESS"))
    });
});
