import { _common, _estimatePage, _validate, _mainView, _boqPage, _bidPage, _saleContractPage, _modalView, _salesPage, _wipPage, _package, _schedulePage, _rfqPage, _projectPage, _businessPartnerPage, _materialPage, _loginPage, _procurementContractPage } from "cypress/pages";
...
const ATTRIBUTE = "ATTR-" + Cypress._.random(0, 999);

let CONTAINERS_DATA_TYPES, CONTAINERS_DATA_RECORDS, CONTAINERS_MATERIAL_CATALOGS, CONTAINERS_MATERIAL_CATALOG_FILTER, CONTAINERS_MATERIAL_RECORDS, CONTAINERS_ATTRIBUTE_VALUES;

let CONTAINER_COLUMNS_MATERIAL_CATALOGS, CONTAINER_COLUMNS_MATERIAL_GROUPS, CONTAINER_COLUMNS_MATERIAL_RECORDS, CONTAINER_COLUMNS_MATERIAL_CATALOG_FILTER, CONTAINER_COLUMNS_ATTRIBUTES, CONTAINER_COLUMNS_ATTRIBUTE_VALUES, CONTAINER_COLUMNS_MATERIAL_ATTRIBUTES;
...
ALLURE.story("PCM- 2.153 | Define Attributes for a material record")
describe("PCM- 2.153 | Define Attributes for a material record", () => {

    before(function () {

        cy.fixture("pcm/pcm-2.153-define-attributes-for-a-material-record.json").then((data) => {
            this.data = data
            ...
            CONTAINER_COLUMNS_ATTRIBUTES = this.data.CONTAINER_COLUMNS.ATTRIBUTES
            CONTAINERS_ATTRIBUTE_VALUES = this.data.CONTAINERS.ATTRIBUTE_VALUES
            CONTAINER_COLUMNS_ATTRIBUTE_VALUES = this.data.CONTAINER_COLUMNS.ATTRIBUTE_VALUES
            CONTAINER_COLUMNS_MATERIAL_ATTRIBUTES = this.data.CONTAINER_COLUMNS.MATERIAL_ATTRIBUTES
    ...
    it('TC - Verify create material catalog and Material groups', function () {
        ...
        _common.openTab(app.TabBar.ATTRIBUTES).then(() => {
            _common.select_tabFromFooter(cnt.uuid.ATTRIBUTE, app.FooterTab.ATTRIBUTES, 2);
            _common.setup_gridLayout(cnt.uuid.ATTRIBUTE, CONTAINER_COLUMNS_ATTRIBUTES)
        });
        _common.create_newRecord(cnt.uuid.ATTRIBUTE)
        _common.set_cellCheckboxValue(cnt.uuid.ATTRIBUTE, app.GridCells.HAS_FIXED_VALUES, CommonLocators.CommonKeys.GRID)
        _common.enterRecord_inNewRow(cnt.uuid.ATTRIBUTE, app.GridCells.PROPERTY_INFO, app.InputFields.DOMAIN_TYPE_TRANSLATION, ATTRIBUTE)
        cy.SAVE().wait(2000)
        _common.openTab(app.TabBar.ATTRIBUTES).then(() => {
            _common.select_tabFromFooter(cnt.uuid.ATTRIBUTE_VALUES, app.FooterTab.ATTRIBUTES_VALUES, 4);
        ...
        _common.enterRecord_inNewRow(cnt.uuid.ATTRIBUTE_VALUES, app.GridCells.CHARACTERISTIC_INFO, app.InputFields.DOMAIN_TYPE_TRANSLATION, CONTAINERS_ATTRIBUTE_VALUES.ATTRIBUTE_VALUE[0])
        cy.SAVE().wait(1000)
    })
    ...
    it('TC - Verify create material record-2', function () {
        _common.openTab(app.TabBar.RECORDS).then(() => {
    ...
    it('TC - Verify create material attributes and values', function () {
        _common.openTab(app.TabBar.RECORDS).then(() => {
            _common.select_tabFromFooter(cnt.uuid.MATERIAL_ATTRIBUTE, app.FooterTab.ATTRIBUTES, 2)
            _common.setup_gridLayout(cnt.uuid.MATERIAL_ATTRIBUTE, CONTAINER_COLUMNS_MATERIAL_ATTRIBUTES)
        });
        _common.select_rowInContainer(cnt.uuid.MATERIAL_ATTRIBUTE)
        _common.delete_recordFromContainer(cnt.uuid.MATERIAL_ATTRIBUTE)
        cy.SAVE()
        _common.create_newRecord(cnt.uuid.MATERIAL_ATTRIBUTE)
        //by caret
        _validate.verify_dataUnderCaret(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.PROPERTY_DESCRIPTION, CommonLocators.CommonKeys.LIST, ATTRIBUTE)
        ...
        _validate.verify_dataUnderCaret(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.CHARACTERISTIC_DESCRIPTION, CommonLocators.CommonKeys.LIST, CONTAINERS_ATTRIBUTE_VALUES.ATTRIBUTE_VALUE[0])
        _common.edit_dropdownCellWithCaret(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.CHARACTERISTIC_DESCRIPTION, CommonLocators.CommonKeys.LIST, CONTAINERS_ATTRIBUTE_VALUES.ATTRIBUTE_VALUE[0])
        cy.SAVE().wait(1000)
        _common.select_rowInContainer(cnt.uuid.MATERIAL_ATTRIBUTE)
        _common.delete_recordFromContainer(cnt.uuid.MATERIAL_ATTRIBUTE)
        cy.SAVE()
        _common.create_newRecord(cnt.uuid.MATERIAL_ATTRIBUTE)
        //by input
        _validate.verify_dataUnderFilter(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.PROPERTY_DESCRIPTION, app.InputFields.INPUT_GROUP_CONTENT, CommonLocators.CommonKeys.LIST, ATTRIBUTE)
        _common.edit_dropdownCellWithInput(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.PROPERTY_DESCRIPTION, CommonLocators.CommonKeys.LIST, app.InputFields.INPUT_GROUP_CONTENT, ATTRIBUTE)
        _common.enterRecord_inNewRow(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.CHARACTERISTIC_DESCRIPTION, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_ATTRIBUTE_VALUES.ATTRIBUTE_VALUE[1])
        cy.SAVE().wait(1000)
        _common.assert_cellData_insideActiveRow(cnt.uuid.MATERIAL_ATTRIBUTE, app.GridCells.CHARACTERISTIC_DESCRIPTION, CONTAINERS_ATTRIBUTE_VALUES.ATTRIBUTE_VALUE[1])
    })
})