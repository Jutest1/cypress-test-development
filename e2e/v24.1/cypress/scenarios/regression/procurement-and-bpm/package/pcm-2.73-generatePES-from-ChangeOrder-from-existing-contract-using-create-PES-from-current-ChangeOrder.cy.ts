...
allure.story("PCM- 2.73 |  Generate a PES from a change order of an existing approved contract, using selection 'Create new PES from current change order'");

describe("PCM- 2.73 |  Generate a PES from a change order of an existing approved contract, using selection 'Create new PES from current change order'", () => {
      before(function () {
        cy.fixture('pcm/pcm-2.73-generate-a-PES-from-a-change-order-of-an-existing-approved-contract-using-selection-create-new-PES-from-current-change-order.json').then((data) => {
                 this.data = data;
  ...
  it("TC - Create boq package from wizards option", function () {
    ...
    _common.waitForLoaderToDisappear()
    _package.enterRecord_toCreateBoQPackage_FromWizard_Duplicate(commonLocators.CommonKeys.PROJECT_BOQ,  commonLocators.CommonLabels.ENTIRE_ESTIMATE,0,commonLocators.CommonKeys.PROJECT_BOQ, "D","BoQ Quantity (WQ/AQ)")
    _common.waitForLoaderToDisappear()
...
_common.waitForLoaderToDisappear()
  ...
  it('TC - Verify PES other relate fields should get value from selected contract;', function () {
