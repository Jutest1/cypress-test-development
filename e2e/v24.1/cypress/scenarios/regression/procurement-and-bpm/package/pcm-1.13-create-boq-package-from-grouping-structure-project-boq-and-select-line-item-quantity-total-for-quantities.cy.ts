...
ALLURE.story("PCM- 1.13 | Create BOQ package from Grouping structure,Project BOQ & select Line item quantity total for Quantities");
describe("PCM- 1.13 | Create BOQ package from Grouping structure,Project BOQ & select Line item quantity total for Quantities", () => {


  before(function () {

    cy.fixture("pcm/pcm-1.13-create-boq-package-from-grouping-structure-project-boq-and-select-line-item-quantity-total-for-quantities.json")
      .then((data) => {
        this.data = data;
  ...
  it("TC - Assign resource to generated line item", function () {
    ...
    _common.saveCellDataToEnv(cnt.uuid.ESTIMATE_LINEITEMS,app.GridCells.QUANTITY_TOTAL,"QUANTITY_TOTAL")
  ...
  it('TC - Verify outline specification, quantity, final price of package BoQ is equal to details from line item', function () {
    ...
    _common.assert_forNumericValues(cnt.uuid.BOQ_STRUCTURE, app.GridCells.QUANTITY_SMALL, Cypress.env("QUANTITY_TOTAL"))
    _common.assert_forNumericValues(cnt.uuid.BOQ_STRUCTURE, app.GridCells.FINAL_PRICE_SMALL, Cypress.env("COSTTOTAL"))
    _common.assert_cellData_insideActiveRow(cnt.uuid.BOQ_STRUCTURE,app.GridCells.BRIEF_INFO_SMALL,BOQ_STRUCTURE_DESC)
  })
});
