...
import { BOQ_ROOT_ITEM, PACKAGE_TOTAL_TRANSLATION } from "cypress/pages/variables";
...
const NEW_BOQ_STRUCTURE = "NEWBOQSTRUCT-1" + Cypress._.random(0, 999);
...
allure.story("PCM- 1.25 | Update package (BoQ) quantities & new item/s from quote");
describe("PCM- 1.25 | Update package (BoQ) quantities & new item/s from quote", () => {
    before(function () {
        cy.fixture("pcm/pcm-1.25-update-package-BoQ-quantities-and-new-items-from-quote.json").then((data) => {
            this.data = data
    ...
    it('TC - Create Quote by wizard', function () {
        ...
        _common.enterRecord_inNewRow(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.BRIEF_INFO_SMALL, app.InputFields.DOMAIN_TYPE_TRANSLATION, NEW_BOQ_STRUCTURE)
        ...
        cy.wait(2000)
        _validate.verify_isRecordMultiplyTwoValuesInRow(cnt.uuid.QUOTEBOQSTRUCTURE, CONTAINERS_BOQ_STRUCTURE.QUOTE_BOQ_QUANTITY, CONTAINERS_BOQ_STRUCTURE.QUOTE_BOQ_UNIT_RATE, app.GridCells.FINAL_PRICE_SMALL)
        _common.saveCellDataToEnv(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.FINAL_PRICE_SMALL, FINAL_PRICE)
    });

    it('TC - Verify new created BoQ Structure Item in Package', function () {
        _common.openTab(app.TabBar.QUOTES).then(() => {
            _common.select_tabFromFooter(cnt.uuid.QUOTES_TOTALS, app.FooterTab.TOTALS, 3);
            _common.setup_gridLayout(cnt.uuid.QUOTES_TOTALS, CONTAINER_COLUMNS_TOTALS)
            _common.clickOn_cellHasUniqueValue(cnt.uuid.QUOTES_TOTALS, app.GridCells.TRANSLATED, CONTAINER_TOTALS.CELL_VALUE, PACKAGE_TOTAL_TRANSLATION)
            _common.assert_cellData_insideActiveRow(cnt.uuid.QUOTES_TOTALS, app.GridCells.VALUE_NET, Cypress.env(FINAL_PRICE))
        });
        ...
        _common.openTab(app.TabBar.BOQBASED).then(() => {
            ...
            _common.setup_gridLayout(cnt.uuid.BOQ_STRUCTURE, CONTAINER_COLUMNS_PACKAGE_BOQ_STRUCTURE)
            _common.clickOn_cellHasUniqueValue(cnt.uuid.BOQ_STRUCTURE, app.GridCells.BRIEF_INFO_SMALL, NEW_BOQ_STRUCTURE)
            _common.assert_cellData_insideActiveRow(cnt.uuid.BOQ_STRUCTURE, app.GridCells.FINAL_PRICE_SMALL, Cypress.env(FINAL_PRICE))
        });
    })
})