...
const BOQ_STRUCTURE_DESC="BOQ_STR_DESC-" + Cypress._.random(0, 999);
...
let MODAL_UPDATE_BOQ_PACKAGE
let CONTAINER_COLUMNS_CONTRACT_BOQ_STRUCTURE
let CONTAINER_COLUMNS_CONTRACT_BOQ
let CONTAINER_COLUMNS_CONTRACT

let CONTAINER_COLUMNS_PACKAGE_BOQ
let CONTAINER_COLUMNS_PACKAGE_BOQ_STRUCTURE

let CONTAINERS_CONTRACT_BOQ_STRUCTURE

const CON_BOQ_STRUCTURE_DESC="CON_BOQ_STR-" + Cypress._.random(0, 999);
const FINALPRICE="Final_Price",FINALGROSS="Final_Gross_Price"

ALLURE.epic("PROCUREMENT AND BPM");
ALLURE.feature("Package");
ALLURE.story("PCM- 1.81 | Update package (BoQ) quantities & new item/s from contract");

describe("PCM- 1.81 | Update package (BoQ) quantities & new item/s from contract", () => {
    before(function () {
        cy.fixture("pcm/pcm-1.81-update-package-boq-quantities-and-new-item-from-contract.json")
          .then((data) => {
            this.data = data;
    ...
    it("TC - Add BoQ structure under contract", function () {
        ...
        _common.openTab(app.TabBar.PROCUREMENTCONTRACTBOQ).then(() => {
            ...
            _common.setup_gridLayout(cnt.uuid.PROCUREMENTCONTRACT_BOQS,CONTAINER_COLUMNS_CONTRACT_BOQ)
        });
        ...
        _common.clickOn_cellHasUniqueValue(cnt.uuid.PROCUREMENTCONTRACT_BOQS,app.GridCells.BRIEF_INFO_SMALL,BOQ_DESC)
        ...
        _common.openTab(app.TabBar.PROCUREMENTCONTRACTBOQ).then(() => {
            ...
            _common.setup_gridLayout(cnt.uuid.PROCUREMENTCONTRACT_BOQSTRUCTURE,CONTAINER_COLUMNS_CONTRACT_BOQ_STRUCTURE)
        });
        ...
        _common.clickOn_toolbarButton(cnt.uuid.PROCUREMENTCONTRACT_BOQSTRUCTURE,btn.ToolBar.ICO_TREE_EXPAND_ALL)
        _boqPage.enterRecord_toCreateBoQStructureUnderpackage(cnt.uuid.PROCUREMENTCONTRACT_BOQSTRUCTURE,CON_BOQ_STRUCTURE_DESC,CONTAINERS_CONTRACT_BOQ_STRUCTURE.QUANTITY,CONTAINERS_CONTRACT_BOQ_STRUCTURE.UNIT_RATE,CONTAINERS_CONTRACT_BOQ_STRUCTURE.UOM)
        _common.waitForLoaderToDisappear()
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        ...
        _common.clear_subContainerFilter(cnt.uuid.PROCUREMENTCONTRACT_BOQS)
        _common.clickOn_cellHasUniqueValue(cnt.uuid.PROCUREMENTCONTRACT_BOQS,app.GridCells.BRIEF_INFO_SMALL,BOQ_DESC)
        _common.saveCellDataToEnv(cnt.uuid.PROCUREMENTCONTRACT_BOQS,app.GridCells.FINAL_PRICE_SMALL,FINALPRICE,BOQ_ROOT_ITEM)
        _common.saveCellDataToEnv(cnt.uuid.PROCUREMENTCONTRACT_BOQS,app.GridCells.FINAL_PRICE_GROSS,FINALGROSS,BOQ_ROOT_ITEM)
        _common.minimizeContainer(cnt.uuid.PROCUREMENTCONTRACT_BOQS)
    })

    it("TC - Update package BoQ", function () {
        ...
        _common.clickOn_goToButton_toSelectModule(cnt.uuid.PROCUREMENTCONTRACT,commonLocators.CommonKeys.PACKAGE)
        _common.waitForLoaderToDisappear()
    ...
    it("TC - Verify BoQ quantity,final price and final price gross under package", function () {
        _common.waitForLoaderToDisappear()
            ...
            _common.setup_gridLayout(cnt.uuid.PROCUREMENT_BOQS,CONTAINER_COLUMNS_PACKAGE_BOQ)
        })
        ...
        _common.maximizeContainer(cnt.uuid.PROCUREMENT_BOQS)
        ...
        _common.assert_forNumericValues(cnt.uuid.PROCUREMENT_BOQS,app.GridCells.FINAL_PRICE_SMALL,Cypress.env(FINALPRICE),BOQ_ROOT_ITEM)
        _common.assert_forNumericValues(cnt.uuid.PROCUREMENT_BOQS,app.GridCells.FINAL_PRICE_GROSS,Cypress.env(FINALGROSS),BOQ_ROOT_ITEM) 
        _common.minimizeContainer(cnt.uuid.PROCUREMENT_BOQS)
        ...
        _common.clickOn_cellHasUniqueValue(cnt.uuid.BOQ_STRUCTURE,app.GridCells.BRIEF_INFO_SMALL,CON_BOQ_STRUCTURE_DESC)
        _common.clickOn_activeRowCell(cnt.uuid.BOQ_STRUCTURE,app.GridCells.BOQ_LINE_TYPE_FK)
        _common.assert_forNumericValues(cnt.uuid.BOQ_STRUCTURE,app.GridCells.QUANTITY_SMALL,CONTAINERS_CONTRACT_BOQ_STRUCTURE.QUANTITY)
