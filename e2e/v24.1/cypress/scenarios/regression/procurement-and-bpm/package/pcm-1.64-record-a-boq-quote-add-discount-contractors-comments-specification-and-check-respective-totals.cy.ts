...
const COMMENT_CONTRACTOR = "COMMENT_CONTRACTOR-" + Cypress._.random(0, 999)
const RFQ_CODE = 'RFQ_CODE'
...
let CONTAINER_COLUMNS_BOQS,
    ...
    CONTAINER_QUOTE,
    CONTAINER_COLUMNS_REQUISITION_BOQ,
    CONTAINER_COLUMNS_QUOTE


allure.epic("PROCUREMENT AND BPM");
allure.feature("Package");
allure.story("PCM- 1.64 | Record a BOQ quote, add discount, contractors comments, specification and check respective totals");

describe("PCM- 1.64 | Record a BOQ quote, add discount, contractors comments, specification and check respective totals", () => {
    before(function () {
        cy.fixture("pcm/pcm-1.64-record-a-boq-quote-add-discount-contractors-comments-specification-and-check-respective-totals.json").then((data) => {
            this.data = data
    ...
    it('TC - Create requisistion by wizard', function () {
        ...
        _common.edit_containerCell(cnt.uuid.REQUISITIONBOQSTRUCTURE, app.GridCells.COMMENT_CONTRACTOR, app.InputFields.DOMAIN_TYPE_REMARK, COMMENT_CONTRACTOR)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.saveCellDataToEnv(cnt.uuid.REQUISITIONBOQSTRUCTURE, app.GridCells.QUANTITY_SMALL, "REQ_BOQ_QUANTITY")
        _common.saveCellDataToEnv(cnt.uuid.REQUISITIONBOQSTRUCTURE, app.GridCells.COMMENT_CONTRACTOR, "CONTRACTOR_COMMENT")
        _common.saveCellDataToEnv(cnt.uuid.REQUISITIONBOQSTRUCTURE, app.GridCells.BRIEF_INFO_SMALL, "SPECIFICATION")
        cy.wait(1000) //required wait to load page
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD)
    ...
    it('TC - Create quote manually', function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
        ...
        _package.enterRecord_toCreateQuote(Cypress.env(RFQ_CODE), CONTAINER_QUOTE.BUSINESS_PARTNER)
        cy.SAVE()
        cy.wait(2000) //required wait to load page
        _common.openTab(app.TabBar.QUOTEBOQS).then(function () {
            _common.select_tabFromFooter(cnt.uuid.QUOTEBOQSTRUCTURE, app.FooterTab.BOQ_STRUCTURE, 3);
            _common.setup_gridLayout(cnt.uuid.QUOTEBOQSTRUCTURE, CONTAINER_COLUMNS_QUOTE)
        })
        cy.wait(2000) //required wait to load page
        ...
        _common.edit_containerCell(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.DISCOUNT_PERCENT, app.InputFields.INPUT_GROUP_CONTENT, CONTAINER_QUOTE.DISCOUNT_PERCENT)
    ...
    it('TC - Verify bidder can input unit rate to BOQ items and give discount, the totals will calculated according input', function () {
        _common.saveNumericCellDataToEnv(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.QUANTITY_SMALL, "Quantity")
        cy.wait(500).then(() => {
            cy.wait(1000) //required wait to load page
            var PRICE = Cypress.env("Quantity") * CONTAINER_QUOTE.UNIT_RATE
            var DISCOUNT = CONTAINER_QUOTE.DISCOUNT_PERCENT / 100
            var DISCOUNT_VALUE = PRICE * DISCOUNT
            var FINAL_PRICE = PRICE - DISCOUNT_VALUE
            _common.assert_forNumericValues(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.FINAL_PRICE_SMALL, FINAL_PRICE.toString())
        })
    });

    it('TC - Verify boq attribute include quantity, specfication and contractor comments are inherit from REQ', function () {
        cy.wait(2000).then(() => {
            _common.assert_cellData_insideActiveRow(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.QUANTITY_SMALL, Cypress.env("REQ_BOQ_QUANTITY"))
            _common.assert_cellData_insideActiveRow(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.COMMENT_CONTRACTOR, Cypress.env("CONTRACTOR_COMMENT"))
            _common.assert_cellData_insideActiveRow(cnt.uuid.QUOTEBOQSTRUCTURE, app.GridCells.BRIEF_INFO_SMALL, Cypress.env("SPECIFICATION"))
