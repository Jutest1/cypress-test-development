...
allure.story("PCM- 1.39 | Price comparison BoQ-Check totals");
describe("PCM- 1.39 | Price comparison BoQ-Check totals", () => {
  
    before(function () {
        cy.fixture("pcm/pcm-1.39-price-comparison-BoQ-check-totals.json").then((data) => {
            this.data = data
    ...
    it("TC - Verify the Price comparison for each Business Partner associated to Quote", function () {
        ...
        _validate.verify_isTotalsInPriceComparisonEqualToFinalPrice(cnt.uuid.PRICE_COMPARISONBOQ, commonLocators.CommonElements.STYLE_RED, Cypress.env(FINAL_PRICE_BP1))
        _validate.verify_isTotalsInPriceComparisonEqualToFinalPrice(cnt.uuid.PRICE_COMPARISONBOQ, commonLocators.CommonElements.STYLE_GREEN, Cypress.env(FINAL_PRICE_BP2))
    })
})