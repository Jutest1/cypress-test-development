...
ALLURE.story("PCM- 1.12 | Create BOQ package from 'Grouping structure' - 'Project BOQ' & select Line item WQ for Quantities");

describe("PCM- 1.12 | Create BOQ package from 'Grouping structure' - 'Project BOQ' & select Line item WQ for Quantities", () => {
  before(function () {
    cy.fixture("pcm/pcm-1.12-create-boq-package-from-grouping-structure-project-boq-&-select-line-item-wq-for-quantities.json")
      .then((data) => {
        this.data = data;
  ...
  it("TC - Generate boq line item", function () {
    ...
    _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
      ...
      _common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.costtotal,CONTAINER_COLUMNS_LINE_ITEM.wqquantitytarget],cnt.uuid.ESTIMATE_LINEITEMS)
    });
