...
describe("PCM- 1.34 | Update package boq quantities and new items from requisitions", () => {
    ...
    it("TC - Create BoQ package and change Package Status", function () {
        ...
        _boqPage.enterRecord_toCreateBoQStructureUnderpackage(cnt.uuid.REQUISITIONBOQSTRUCTURE, PACKAGEBOQESC, CONTAINER_PACKAGE_BOQ.QUANTITY, CONTAINER_PACKAGE_BOQ.UNIT_RATE, CONTAINER_PACKAGE_BOQ.UOM)
        cy.SAVE()
        cy.wait(1000) //required wait to load page
    ...
    it("TC - Verify unit rate, quantity, final price, Outline Specification in package boq structure", function () {
        _common.openTab(app.TabBar.PACKAGE).then(() => {
            ...
            _common.set_columnAtTop([CONTAINER_COLUMNS_PACKAGE_BOQ.quantity, CONTAINER_COLUMNS_PACKAGE_BOQ.price, CONTAINER_COLUMNS_PACKAGE_BOQ.finalprice, CONTAINER_COLUMNS_PACKAGE_BOQ.briefinfo], cnt.uuid.BOQ_STRUCTURE)
        });
        _common.maximizeContainer(cnt.uuid.BOQ_STRUCTURE);
        cy.REFRESH_CONTAINER()
        _common.waitForLoaderToDisappear()
        _common.clickOn_cellHasUniqueValue(cnt.uuid.BOQ_STRUCTURE, app.GridCells.BRIEF_INFO_SMALL, PACKAGEBOQESC)
