...
allure.story("PCM- 2.119 |  Create a new invoice and test header Data after it is generated from wizard");
describe("PCM- 2.119 |  Create a new invoice and test header Data after it is generated from wizard", () => {
  beforeEach(function () {
    cy.fixture("pcm/pcm-2.119-Create-a-new-invoice-and-test-header-Data-after-it-is-generated-from-wizard.json").then((data) => {
      this.data = data;
    });
  });
  before(function () {
  ...
  it('TC - NAvigate to Customizing module and select Invoice type', function () {
    ...
    _common.search_inSubContainer(cnt.uuid.DATA_TYPES, CONTAINERS_DATA_TYPES.INVOICE_TYPE);
    cy.REFRESH_CONTAINER();
    _common.clickOn_cellHasUniqueValue(cnt.uuid.DATA_TYPES, app.GridCells.MODULE_NAME, CONTAINERS_DATA_TYPES.INVOICE);
    cy.SAVE();
    cy.wait(2000)
    _common.openTab(app.TabBar.MASTER_DATA).then(() => {
    ...
    cy.wait(1000)
    _common.clickOn_cellHasUniqueValue(cnt.uuid.DATA_RECORDS, app.GridCells.DESCRIPTION_INFO, CONTAINERS_DATA_RECORDS.DESCRIPTION);
  ...
  it('TC - Navigate to Procurement Configuration module and select configuration', function () {
		_common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART);
    ...
    _common.clickOn_cellHasUniqueValue(cnt.uuid.CONFIGURATION_HEADER, app.GridCells.DESCRIPTION_INFO, CONTAINERS_CONFIGURATION_HEADER.DESCRIPTIONINFO)
    ...
    _common.select_dataFromSubContainer(cnt.uuid.CONFIGURATIONS, CONTAINERS_CONFIGURATIONS.INVOICES)
    cy.wait(2000)
    _common.select_dataFromSubContainer(cnt.uuid.CONFIGURATIONS, CONTAINERS_CONFIGURATIONS.SERVICE_PA)
    cy.wait(2000)
    _common.set_cellCheckboxValue( cnt.uuid.CONFIGURATIONS, app.GridCells.IS_DEFAULT,commonLocators.CommonKeys.CHECK)
  })
  ...
  it("TC - Verify Invoice header records", function () {
    ...
    _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICEHEADER, app.GridCells.BUSINESS_PARTNER_FK, CONTAINERS_PROCUREMENTCONTRACT.BUISNESS_PARTNER)
    _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICEHEADER, app.GridCells.PRC_CONFIGURATION_FK, CONTAINERS_CONFIGURATIONS.SERVICE_PA)
    _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICEHEADER, app.GridCells.INV_TYPE_FK, CONTAINERS_DATA_RECORDS.DESCRIPTION)

    _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICEHEADER, app.GridCells.CON_HEADER_FK, Cypress.env("ContractCode"))
    _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICEHEADER, app.GridCells.BILLING_SCHEMA_FK, Cypress.env("BILLINGSCHEMA"))
    _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICEHEADER, app.GridCells.PAYMENT_TERM_FK, Cypress.env("PAYMENT_TERM_FI"))
  ...
  it("TC - Verify Performance Entry Sheets container record", function () {
    ...
    _common.assert_cellData_insideActiveRow(cnt.uuid.PERFORMANCE_ENTRY_SHEETS,app.GridCells.PES_HEADER_FK, Cypress.env("PES_ENV"))
    _common.waitForLoaderToDisappear()
    cy.wait(1000)//required Waits
  })
  ...
  it("TC - Verify reconciliation calculation is correct", function () {
    ...
    _common.assert_forNumericValues(cnt.uuid.RECONCILIATION, app.GridCells.RECON_GROSS_CO, Cypress.env("PES_ENV_TG"))
    _common.assert_forNumericValues(cnt.uuid.RECONCILIATION, app.GridCells.RECON_VAT, Cypress.env("PES_ENV_VAT"))
    _common.assert_forNumericValues(cnt.uuid.RECONCILIATION, app.GridCells.RECON_NET, Cypress.env("PES_ENV_OC"))
  })

  it("TC - Verify Billing Schema calculation is correct", function () {
    _common.openTab(app.TabBar.APPLICATION).then(() => {
      _common.select_tabFromFooter(cnt.uuid.INVOICE_BILLING_SCHEMA, app.FooterTab.BILLING_SCHEMA, 3)
      _common.clear_subContainerFilter(cnt.uuid.INVOICE_BILLING_SCHEMA)
      _common.search_inSubContainer(cnt.uuid.INVOICE_BILLING_SCHEMA,' ')
      _common.assert_cellDataByContent_inContainer(cnt.uuid.INVOICE_BILLING_SCHEMA, app.GridCells.RESULT, Cypress.env("PES_ENV_OC"))
    })
  })
