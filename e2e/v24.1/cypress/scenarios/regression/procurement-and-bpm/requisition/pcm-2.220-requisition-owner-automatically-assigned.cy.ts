...
allure.story("PCM- 2.220 | Requisition owner - Automatically assigned")

describe("PCM- 2.220 | Requisition owner - Automatically assigned", () => {
    before(function () {
        cy.fixture("pcm/pcm-2.220-requisition-owner-automatically-assigned.json").then((data) => {
            this.data = data;
    ...
    it("TC - Adding material in cart and placing an order to create a requisition", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
    ...
    it("TC - Verify default to login user clerk", function () {
        cy.wait(2000)
        _common.assert_labelValues_fromModal(commonLocators.CommonElements.ROW, 'Requisition Owner', app.InputFields.INPUT_GROUP_CONTENT, CONTAINER_CLERK.CODE)
    })

    it("TC - Verify allow user to change it", function () {
