...
let CONTAINER_COLUMNS_BOQS,
    ...
    MODAL_UPDATE_ESTIMATE_WIZARD,CONTAINER_COLUMNS_PROCUREMENT_BOQ,
    CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE,CONTAINERS_REQUISITION_BOQ_STRUCTURE


const PROCBOQ_DESC="PROCBOQ_DESC-"+ Cypress._.random(0, 999);
const REQBOQSTR_DESC="REQBOQSTR_DESC-"+ Cypress._.random(0, 999);


allure.epic("PROCUREMENT AND BPM");
allure.feature("Quote");
allure.story("PCM- 1.65 | Check Quotes creation for one and for several BoQs");

describe("PCM- 1.65 | Check Quotes creation for one and for several BoQs", () => {
    before(function () {
    cy.fixture('pcm/pcm-1.65-check-quotes-creation-for-one-and-for-several-boqs.json').then((data) => {
        this.data = data;
    ...
    it("TC - Create requisition and change requisition status", function () {
        ...
        _common.openTab(app.TabBar.MAIN).then(() => {
            _common.setDefaultView(app.TabBar.MAIN)
            _common.waitForLoaderToDisappear(); 
            _common.select_tabFromFooter(cnt.uuid.REQUISITIONPROCUREMENTBOQS, app.FooterTab.PROCUREMENT_BOQ, 0);
        });
        _common.create_newRecord(cnt.uuid.REQUISITIONPROCUREMENTBOQS)
        _boqPage.enterRecord_ToCreate_procurementBoQs( commonLocators.CommonKeys.SERVICE, PROCBOQ_DESC,commonLocators.CommonLabels.CREATE_NEW_BOQ)
        cy.SAVE()
        _common.waitForLoaderToDisappear();   
            
        _common.openTab(app.TabBar.MAIN).then(() => {
            _common.select_tabFromFooter(cnt.uuid.REQUISITIONBOQSTRUCTURE, app.FooterTab.BOQ_STRUCTURE, 3);
            _common.setup_gridLayout(cnt.uuid.REQUISITIONBOQSTRUCTURE, CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE)
            _common.set_columnAtTop([CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE.quantity,CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE.price,CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE.briefinfo,CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE.boqlinetypefk,CONTAINER_COLUMNS_REQUISITION_BOQ_STRUCTURE.basuomfk],cnt.uuid.REQUISITIONBOQSTRUCTURE)
        });
        _common.waitForLoaderToDisappear();
        _common.maximizeContainer(cnt.uuid.REQUISITIONBOQSTRUCTURE)
        _rfqPage.enterRecordToCreate_BoQStructureInRequisition(REQBOQSTR_DESC, CONTAINERS_REQUISITION_BOQ_STRUCTURE.QUANTITY, CONTAINERS_REQUISITION_BOQ_STRUCTURE.UNIT_RATE, CONTAINERS_REQUISITION_BOQ_STRUCTURE.UOM)
        cy.SAVE()
        _common.waitForLoaderToDisappear(); 
    ...
    it("TC - Create RFQ and change RFQ status", function () {
    ...
    it("TC - Create quote", function () {
        ...
        _common.search_inSubContainer(cnt.uuid.QUOTEPROCUREMENTBOQS,BOQ_DESC)
        _common.select_rowInContainer(cnt.uuid.QUOTEPROCUREMENTBOQS)
        _common.assert_cellData_insideActiveRow(cnt.uuid.QUOTEPROCUREMENTBOQS,app.GridCells.BRIEF_INFO_SMALL,BOQ_DESC)
        _common.openTab(app.TabBar.QUOTES).then(()=>{            
        ...
        cy.wait(1000)
        _common.assert_forNumericValues(cnt.uuid.QUOTEBOQSTRUCTURE,app.GridCells.QUANTITY_SMALL,CONTAINERS_BOQ_STRUCTURE.BOQ_STR_QUANTITY)
        _common.openTab(app.TabBar.QUOTES).then(()=>{            
            _common.select_tabFromFooter(cnt.uuid.QUOTEPROCUREMENTBOQS,app.FooterTab.PROCUREMENT_BOQ,0)
        })
        _common.clear_subContainerFilter(cnt.uuid.QUOTEPROCUREMENTBOQS)
        _common.clickOn_cellHasUniqueValue(cnt.uuid.QUOTEPROCUREMENTBOQS,app.GridCells.REFERENCE,Cypress.env("Text"))
        _common.assert_cellData_insideActiveRow(cnt.uuid.QUOTEPROCUREMENTBOQS,app.GridCells.REFERENCE,Cypress.env("Text"))
        _common.select_rowInContainer(cnt.uuid.QUOTEPROCUREMENTBOQS)
        _common.openTab(app.TabBar.QUOTES).then(()=>{            
            _common.select_tabFromFooter(cnt.uuid.QUOTEBOQSTRUCTURE,app.FooterTab.BOQ_STRUCTURE,3)
        })
        _common.clickOn_cellHasIcon(cnt.uuid.QUOTEBOQSTRUCTURE,app.GridCells.TREE,app.GridCellIcons.ICO_BOQ_ITEM)
         cy.wait(1000)
        _common.assert_forNumericValues(cnt.uuid.QUOTEBOQSTRUCTURE,app.GridCells.QUANTITY_SMALL,CONTAINERS_REQUISITION_BOQ_STRUCTURE.QUANTITY)
    })
