...
allure.epic("QTO");
allure.feature("QTO");
allure.story("QTO-1.4 | Renumber QTO lines");
...
let CONTAINERS_RENUMBER_QTO_DLINES;
let RENUMBER_QTO_DLINES_PARAMETERS:DataCells;
...
describe ('QTO-1.4 | Renumber QTO lines',()=>{
      before(function () {
    ...
    /*renumber qto lines*/
    CONTAINERS_RENUMBER_QTO_DLINES =this.data.CONTAINERS.RENUMBER_QTO_LINES;
    RENUMBER_QTO_DLINES_PARAMETERS={
      [app.GridCells.SHEET]:CONTAINERS_RENUMBER_QTO_DLINES.SHEET,
      [app.GridCells.LINE]:CONTAINERS_RENUMBER_QTO_DLINES.LINE,
      [app.GridCells.INDEX]:CONTAINERS_RENUMBER_QTO_DLINES.INDEX
    }   
        }).then(()=>{
        //Check login
...
cy.wait(1000);
 ...
 it('create qto detail line', function(){
  ...
  it("Renumber QTO lines", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD);
        /* find Update WQ and AQ quantities to BoQ wizard*/
        _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.RENUMBER_QTO_LINES);
        _qtoPage.renumberQTOLinesFromWizard(cnt.uuid.QUANTITY_TAKEOFF_DETAIL, RENUMBER_QTO_DLINES_PARAMETERS);
 
      });

 it('Verify sheet,line,index after renumber',function(){
       _qtoPage.textOfEnvCode(cnt.uuid.QUANTITY_TAKEOFF_DETAIL, app.GridCells.PAGE_NUMBER,'Sheet');
       _qtoPage.textOfEnvCode(cnt.uuid.QUANTITY_TAKEOFF_DETAIL, app.GridCells.LINE_REFERENCE,'Line');
       _qtoPage.textOfEnvCode(cnt.uuid.QUANTITY_TAKEOFF_DETAIL, app.GridCells.LINE_INDEX,'Index');
       cy.wait(500).then(() => {
        expect(Cypress.env("Sheet")).to.eq(CONTAINERS_RENUMBER_QTO_DLINES.SHEET);
        cy.log('The input sheet :'+CONTAINERS_RENUMBER_QTO_DLINES.SHEET+'; The sheet after renumber :'+Cypress.env("Sheet")+';');
        expect(Cypress.env("Line")).to.eq(CONTAINERS_RENUMBER_QTO_DLINES.LINE);
        cy.log('The input line :'+CONTAINERS_RENUMBER_QTO_DLINES.LINE+'; The line after renumber :'+Cypress.env("Line")+'; ');
        expect(Cypress.env("Index")).to.eq(CONTAINERS_RENUMBER_QTO_DLINES.INDEX);
        cy.log('The input index :'+CONTAINERS_RENUMBER_QTO_DLINES.INDEX+'; The index after renumber :'+Cypress.env("Index")+'; ');
      })
      })
