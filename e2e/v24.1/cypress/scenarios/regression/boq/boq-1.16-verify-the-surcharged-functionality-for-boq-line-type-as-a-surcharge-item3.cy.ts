...
ALLURE.epic("BOQ");
ALLURE.feature("BoQ");
ALLURE.story("BOQ- 1.16 | Verify the Surcharged functionality for BOQ Line Type as a Surcharge Item3");

describe("BOQ- 1.16 | Verify the Surcharged functionality for BOQ Line Type as a Surcharge Item3", () => {
  before(function () {
    cy.fixture("boq/boq-1.16-verify-the-surcharged-functionality-for-boq-line-type-as-a-surcharge-item3.json").then((data) => {
      this.data = data;
  ...
  it("TC - Create BoQ Structure Surcharge Item3 boq line type", function () {
    ...
    _common.waitForLoaderToDisappear()
    _common.assert_cellData(cnt.uuid.SURCHARGE_ON,app.GridCells.QUANTITY_SMALL,CONTAINERS_BOQ_STRUCTURE.QUANTITY)
    _common.waitForLoaderToDisappear();
...
it("TC - Verify surcharge item3 'Quantity',surchange on 'Total', and position 'Item Total'", function () {
  _common.select_rowHasValue(cnt.uuid.BOQ_STRUCTURES,BOQ_STRUCT_DESC2)
...
it("TC - Verify surcharge item3 'Item Total'", function () {
  _common.select_rowHasValue(cnt.uuid.BOQ_STRUCTURES,BOQ_STRUCT_DESC2)
