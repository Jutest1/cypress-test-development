...
ALLURE.epic("BOQ");
ALLURE.feature("BoQ");
ALLURE.story("BOQ- 2.1 | Verify cost codes look up from boq module");

describe("BOQ- 2.1 | Verify cost codes look up from boq module", () => {
  before(function () {
    cy.fixture("boq/boq-2.1-verify-cost-codes-look-up-from-boq-module.json")
      .then((data) => {
        this.data = data;
  ...
  it("TC - Verify cost code search in BOQ structure", function () {
      ...
      cy.SAVE();
      _common.waitForLoaderToDisappear()
      _common.lookUpButtonInCell(cnt.uuid.BOQ_STRUCTURES,app.GridCells.MDC_COST_CODE_FK,app.InputFields.ICO_INPUT_LOOKUP,0)
      _common.waitForLoaderToDisappear()
      _validate.verify_costCode_inBoqStructure(CONTAINERS_BOQ_STRUCTURE2.COST_CODE)
      _common.waitForLoaderToDisappear()
  });
})