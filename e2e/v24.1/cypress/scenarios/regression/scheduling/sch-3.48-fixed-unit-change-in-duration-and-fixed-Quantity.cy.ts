...
ALLURE.story("SCH- 3.48 | Fixed Time: Change in Quantity and Fixed Duration ");

describe("SCH- 3.48 | Fixed Time: Change in Quantity and Fixed Duration ", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.48-fixed-unit-change-in-duration-and-fixed-Quantity.json").then((data) => {
        this.data = data
    ...
    it("TC - Verify Productivity =  (Changed Quantity /Changed Duration) ", function () {
        _common.openTab(app.TabBar.PLANNING).then(() => {
        _common.select_tabFromFooter(cnt.uuid.ACTIVITY_STRUCTURE,app.FooterTab.ACTIVITY_STRUCTURE)
         });
         _common.maximizeContainer(cnt.uuid.ACTIVITY_STRUCTURE)
         _common.enterRecord_inNewRow(cnt.uuid.ACTIVITY_STRUCTURE, app.GridCells.QUANTITY_SMALL, app.InputFields.INPUT_GROUP_CONTENT, CONTAINERS_PERFORMANCE_RULES.SCHEDULE_QUANTITY)
         cy.wait(1000)
         cy.SAVE()
         _common.waitForLoaderToDisappear()
        _validate.verify_isRecordDivisionOfTwoValuesAnd_ComapreWithThirdValue(cnt.uuid.ACTIVITY_STRUCTURE,CONTAINERS_PERFORMANCE_RULES.SCHEDULE_QUANTITY,CONTAINERS_PERFORMANCE_RULES.QUANTITY,app.GridCells.PLANNED_DURATION)
