...
ALLURE.story("SCH- 3.60 | calculate the 'Productivity' of the activity by assigning a 'performance sheet' & using the wizard 'Apply Performance sheet' in scheduling by selecting the option `Started` ");

describe("SCH- 3.60 | calculate the 'Productivity' of the activity by assigning a 'performance sheet' & using the wizard 'Apply Performance sheet' in scheduling by selecting the option `Started` ", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.60-calculate-Productivity-of-activity-using-wizard-selecting-option-Started.json").then((data) => {
        this.data = data
    ...
    it("TC -  Set performance %  and click start check box", function () {
       ...
       _common.select_rowHasValue(cnt.uuid.ACTIVITY_STRUCTURE, "m2") 
        _common.set_cellCheckboxValue(cnt.uuid.ACTIVITY_STRUCTURE,app.GridCells.EXECUTION_STARTED,"check")
        cy.wait(1000)
