...
ALLURE.story("SCH- 3.28 | Indent Locations");

describe("SCH- 3.28 | Indent Locations", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.28-indent-locations.json").then((data) => {
            this.data = data
   ...
   it("TC - Indent Locations", function () {
    _common.clickOn_toolbarButton(cnt.uuid.PROJECT_LOCATION, btn.ToolBar.ICO_DEMOTE)       
    _common.search_inSubContainer(cnt.uuid.PROJECT_LOCATION, LOCATION_CODE)
    _common.waitForLoaderToDisappear()    
    _validate.verify_isIconPresent(cnt.uuid.PROJECT_LOCATION, app.GridCells.TREE, app.GridCellIcons.ICO_LOCATION_GROUP)  
    cy.SAVE()    
    ...
    _common.search_inSubContainer(cnt.uuid.PROJECT_LOCATION, LOCATION_CODE1)
    _common.waitForLoaderToDisappear()

    _common.select_rowHasValue(cnt.uuid.PROJECT_LOCATION, LOCATION_CODE1)
    _validate.verify_isIconPresent(cnt.uuid.PROJECT_LOCATION, app.GridCells.TREE, app.GridCellIcons.ICO_LOCATION2)
    cy.wait(1000) //Wait is required to load data.
    _common.assert_cellData_insideActiveRow(cnt.uuid.PROJECT_LOCATION,app.GridCells.DESCRIPTION_INFO,LOCATION_DESC1)
