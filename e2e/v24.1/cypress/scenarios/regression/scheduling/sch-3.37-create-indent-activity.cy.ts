...
ALLURE.story("SCH- 3.37 | Create Indent Activity");

describe("SCH- 3.37 | Create Indent Activity", () => {
    before(function () {   
        cy.fixture("scheduling/sch-3.37-create-indent-activity.json").then((data) => {
         this.data = data
    ...
    it("TC - Indent Activity", function () {
        _common.clickOn_toolbarButton(cnt.uuid.ACTIVITY_STRUCTURE, btn.ToolBar.ICO_DEMOTE)  
        cy.REFRESH_CONTAINER()     
        ...
        _common.waitForLoaderToDisappear()
        _validate.verify_isIconPresent(cnt.uuid.ACTIVITY_STRUCTURE, app.GridCells.TREE, app.GridCellIcons.ICO_TASK_SUMMARY)  
        cy.SAVE()
