...
ALLURE.story("SCH- 3.32 | Create Schedule Activity ");

describe("SCH- 3.32 | Create Schedule Activity ", () => {
    before(function () {   
        cy.fixture("scheduling/sch-3.32-create-schedule-activity.json").then((data) => {
            this.data = data
    ...
    it("TC - Add activity structure and append data by copy paste button ", function () {
    ...
    it("TC - Verify the assertion of Append by context paste button ", function() {
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.ACTIVITY_STRUCTURE,app.GridCells.QUANTITY_SMALL,CONTAINERS_ACTIVITY_STRUCTURE.QUANTITY)
        _common.assert_cellData_insideActiveRow(cnt.uuid.ACTIVITY_STRUCTURE,app.GridCells.QUANTITY_UOM_FK,CONTAINERS_ACTIVITY_STRUCTURE.UOM)
    ...
    it("TC - Edit, Delete the appended Data and verify the deleted data", function () {
        ...
        _common.edit_containerCell(cnt.uuid.ACTIVITY_STRUCTURE,app.GridCells.DESCRIPTION,app.InputFields.DOMAIN_TYPE_COMMENT,ACT_STRUCT_DESC2)
        ...
        _common.waitForLoaderToDisappear()
        _validate.verify_isRecordDeleted(cnt.uuid.ACTIVITY_STRUCTURE, ACT_STRUCT_DESC2)
