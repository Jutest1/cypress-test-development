...
ALLURE.story("SCH- 3.47 | Fixed Time: Change in Quantity and Fixed Duration ");

describe("SCH- 3.47 | Fixed Time: Change in Quantity and Fixed Duration ", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.47-fixed-time-change-in-quantity-and-fixed-duration.json").then((data) => {
        this.data = data
    ...
    it("TC - Verify Productivity =  (Changed Quantity /Fixed Duration) ", function () {
        _common.openTab(app.TabBar.PLANNING).then(() => {
        _common.select_tabFromFooter(cnt.uuid.ACTIVITY_STRUCTURE,app.FooterTab.ACTIVITY_STRUCTURE)
         });
         _common.maximizeContainer(cnt.uuid.ACTIVITY_STRUCTURE)
