...
ALLURE.feature("Delete Baseline");
ALLURE.story("SCH- 3.68 | Delete Baseline");

describe("SCH- 3.68 | Delete Baseline", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.68-delete-baseline.json").then((data) => {
            this.data = data
    ...
    it("TC - Delete the Create Baseline ", function () {
        _common.openTab(app.TabBar.PLANNING).then(() => {
            _common.select_tabFromFooter(cnt.uuid.ACTIVITY_STRUCTURE, app.FooterTab.ACTIVITY_STRUCTURE)
            });
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD);
        _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD,Sidebar.SideBarOptions.DELETE_BASELINE);
        _common.select_multipleRow_fromModal()
        _common.set_cellCheckboxValue_fromModal(app.GridCells.USE_IN_SPLIT,"check")
    ...
    it("TC - Verify the deleted baseline  ", function () {
        _common.openTab(app.TabBar.PLANNING).then(() => {
        ...
        _common.clear_subContainerFilter(cnt.uuid.BASE_LINES)
        _validate.verify_recordNotPresentInContainer(cnt.uuid.BASE_LINES,BASELINE_DESC)
    });
