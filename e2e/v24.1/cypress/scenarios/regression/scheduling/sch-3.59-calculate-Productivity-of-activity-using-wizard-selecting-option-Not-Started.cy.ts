...
let PLANNED_FINISH
let UNITCOLOUMN
let APPLY_PERFORMANCE_PARAMETER
let MODAL_PERFORMANCE_SHEET

ALLURE.epic("SCHEDULING");
ALLURE.feature("Performance sheet ");
ALLURE.story("SCH- 3.59 | calculate the 'Productivity' of the activity by assigning a 'performance sheet' & using the wizard 'Apply Performance sheet' in scheduling by selecting the option `Not Started` ");

describe("SCH- 3.59 | calculate the 'Productivity' of the activity by assigning a 'performance sheet' & using the wizard 'Apply Performance sheet' in scheduling by selecting the option `Not Started` ", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.59-calculate-Productivity-of-activity-using-wizard-selecting-option-Not-Started.json").then((data) => {
        this.data = data
        CONTAINER_COLUMNS_PERFORMANCE_RULES= this.data.CONTAINER_COLUMNS.PERFORMANCE_RULES
         ...
         MODAL_PERFORMANCE_SHEET = this.data.MODAL.APPLY_PERFORMANCE_SHEET_WIZARD
        ...
        APPLY_PERFORMANCE_PARAMETER = {
            [commonLocators.CommonKeys.CHECKBOX]: MODAL_PERFORMANCE_SHEET
        }
       })
    ...
    it("TC -  Apply performance sheet  ", function () {
        ...
        _common.waitForLoaderToDisappear()
        _schedulePage.apply_performance_sheet_fromWizard(APPLY_PERFORMANCE_PARAMETER)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
