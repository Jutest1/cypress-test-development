...
ALLURE.story("SCH- 3.50 |  Fixed Time: Change in Productivity and Fixed Duration ");

describe("SCH- 3.50 |  Fixed Time: Change in Productivity and Fixed Duration ", () => {
    before(function () {
        cy.fixture("scheduling/sch-3.50-fixed-time-change-in-Productivity-and-fixed-duration.json").then((data) => {
        this.data = data
    ...
    it("TC - Define 'Template Activity Properties'.", function () {
       ...
       _common.edit_dropdownCellWithCaret(cnt.uuid.TEMPLATE_ACTIVITY,app.GridCells.TASK_TYPE_FK,commonLocators.CommonKeys.LIST,commonLocators.CommonKeys.FIXED_TIME)
    ...
    it("TC - Verify Quantity = Change Productivity * Fixed Duration ", function () {
        _common.openTab(app.TabBar.PLANNING).then(() => {
        _common.select_tabFromFooter(cnt.uuid.ACTIVITY_STRUCTURE,app.FooterTab.ACTIVITY_STRUCTURE)
         });
         _common.maximizeContainer(cnt.uuid.ACTIVITY_STRUCTURE)
