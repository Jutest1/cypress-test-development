...
let PLANNED_START
let PLANNED_FINISH
let GENERATE_ACTIVITY_PARAMETER
...
ALLURE.story("SCH- 3.96 | Generate Activities in the 'Scheduling' 'module for free schedule with no link to Estimate");

describe("SCH- 3.96 | Generate Activities in the 'Scheduling' 'module for free schedule with no link to Estimate", () => {
    before(function () {   
        cy.fixture("scheduling/sch-3.96-generate-activities-in-the-scheduling-module-for-free-schedules-with-nolink-to-estimate.json").then((data) => {
         this.data = data
        ...
        GENERATE_ACTIVITY_PARAMETER={
            ...
            [commonLocators.CommonKeys.CRITERIA_LABEL]:{"Criteria 1":"Location"}
        };
      }).then(()=>{
    ...
    it('TC - Prerequisite -Adding Generate activity in schedule wizard ', function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
		_common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.MODULES);
        _common.waitForLoaderToDisappear()
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_SEARCH).delete_pinnedItem()
        _common.search_fromSidebar(commonLocators.CommonKeys.STANDARD,"Scheduling")
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.MODULES).then(() => {
        ...
        _common.select_rowHasValue(cnt.uuid.MODULES,"Scheduling")
        _common.clear_subContainerFilter(cnt.uuid.WIZARD_GROUP);
        _common.select_rowHasValue(cnt.uuid.WIZARD_GROUP,"Schedule")
        _common.openTab(app.TabBar.WIZARD).then(() => {
        _common.select_tabFromFooter(cnt.uuid.WIZARD_TO_GROUP, app.FooterTab.WIZARD_TO_GROUP, 1);
          ...
          _common.create_newRecord(cnt.uuid.WIZARD_TO_GROUP)
          _common.edit_dropdownCellWithInput(cnt.uuid.WIZARD_TO_GROUP,app.GridCells.WIZARD_FK,commonLocators.CommonKeys.GRID,app.InputFields.INPUT_GROUP_CONTENT,"Generate Activities")
          cy.wait(2000)//required wait
         _common.enterRecord_inNewRow(cnt.uuid.WIZARD_TO_GROUP,app.GridCells.NAME,app.InputFields.DOMAIN_TYPE_TRANSLATION,"Generate Activities")
          _common.set_cellCheckboxValue(cnt.uuid.WIZARD_TO_GROUP,app.GridCells.IS_VISIBLE,"check")
          cy.SAVE()
          cy.REFRESH_CONTAINER();
          _common.waitForLoaderToDisappear()
          _common.select_rowHasValue(cnt.uuid.WIZARD_GROUP,"Schedule")
          _common.search_inSubContainer(cnt.uuid.WIZARD_TO_GROUP,"Generate Activities")
          _common.assert_cellDataByContent_inContainer(cnt.uuid.WIZARD_TO_GROUP,app.GridCells.NAME,"Generate Activities")   
    ...
    it("TC - Create location records", function () {
        ...
        cy.SAVE()
        _common.saveCellDataToEnv(cnt.uuid.PROJECT_LOCATION,app.GridCells.DESCRIPTION_INFO,"LOCATION_DESCRIPTION1")
        _common.clear_subContainerFilter(cnt.uuid.PROJECT_LOCATION)
        _common.create_newRecord(cnt.uuid.PROJECT_LOCATION)
        ...
        cy.SAVE()
        _common.saveCellDataToEnv(cnt.uuid.PROJECT_LOCATION,app.GridCells.DESCRIPTION_INFO,"LOCATION_DESCRIPTION2")
    });
    ...
    it("TC - Generate Activity by  using Wizard", function () {
        ...
        _common.waitForLoaderToDisappear()
	    _schedulePage.generate_activityStructureRecord_byWizard(GENERATE_ACTIVITY_PARAMETER)
    ...
    it("TC - Verify the generated location in activity structure", function () {
        _common.openTab(app.TabBar.PLANNING).then(() => {
        _common.waitForLoaderToDisappear()
        _common.select_tabFromFooter(cnt.uuid.ACTIVITY_STRUCTURE, app.FooterTab.ACTIVITY_STRUCTURE, 0);
         });
         _common.select_rowHasValue(cnt.uuid.ACTIVITY_STRUCTURE,LOCATION_CODE1)
         _common.assert_cellData_insideActiveRow(cnt.uuid.ACTIVITY_STRUCTURE,app.GridCells.DESCRIPTION,Cypress.env("LOCATION_DESCRIPTION1"))
         _common.select_rowHasValue(cnt.uuid.ACTIVITY_STRUCTURE,LOCATION_CODE2)
         _common.assert_cellData_insideActiveRow(cnt.uuid.ACTIVITY_STRUCTURE,app.GridCells.DESCRIPTION,Cypress.env("LOCATION_DESCRIPTION2"))
    });
