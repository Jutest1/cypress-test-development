...
let CONTAINER_COLUMNS_RELATED_CONTRACTS;

ALLURE.epic("SALES");
ALLURE.feature("Sales-WIP");
ALLURE.story("SAM- 2.8 | Create combined WIP for change order contract");
describe("SAM- 2.8 | Create combined WIP for change order contract", () => {

    before(function () {
        cy.fixture("sam/sam-2.8-Create-combined-WIP-for-change-order-contract.json")
          .then((data) => {
            this.data = data;
            ...
            CONTAINER_COLUMNS_RELATED_CONTRACTS=this.data.CONTAINER_COLUMNS.RELATED_CONTRACTS
    ...
    it("TC - Create WIP and assert the Contract quantity, main and change order in WIP", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART);
        ...
        _common.openTab(app.TabBar.WIP).then(() => {
            ...
            _common.waitForLoaderToDisappear()
            _common.select_tabFromFooter(cnt.uuid.RELATED_CONTRACTS, app.FooterTab.RELATED_CONTRACTS,2);
            _common.setup_gridLayout(cnt.uuid.RELATED_CONTRACTS, CONTAINER_COLUMNS_RELATED_CONTRACTS)
            _common.set_columnAtTop([CONTAINER_COLUMNS_RELATED_CONTRACTS.code],cnt.uuid.RELATED_CONTRACTS)
        })
        _common.clear_subContainerFilter(cnt.uuid.RELATED_CONTRACTS)
        _common.search_inSubContainer(cnt.uuid.RELATED_CONTRACTS,CONTRACT_DESC)
        _common.select_rowHasValue(cnt.uuid.RELATED_CONTRACTS,CONTRACT_DESC) 
        _common.assert_cellData(cnt.uuid.RELATED_CONTRACTS,app.GridCells.CODE,Cypress.env("MAIN_CODE"))
        _common.clear_subContainerFilter(cnt.uuid.RELATED_CONTRACTS)
        _common.search_inSubContainer(cnt.uuid.RELATED_CONTRACTS,CONTRACT_DESC_1)
        _common.select_rowHasValue(cnt.uuid.RELATED_CONTRACTS,CONTRACT_DESC_1) 
        _common.assert_cellData(cnt.uuid.RELATED_CONTRACTS,app.GridCells.CODE,Cypress.env("CHANGE_CODE"))
