...
let MODAL_PROJECTS
let PROJECTS_PARAMETERS_2


allure.epic("SALES");
allure.feature("Sales-Contract");
allure.story("SAM- 1.42 | Delivery Address Field using pull down");
describe("SAM- 1.42 | Delivery Address Field using pull down", () => {

  before(function () {
    cy.preLoading(Cypress.env("adminUserName"), Cypress.env("adminPassword"), Cypress.env("parentCompanyName"), Cypress.env("childCompanyName"));
    cy.fixture("sam/sam-1.42-delivery-address-Field-using-pull-down.json").then((data) => {
      this.data = data;
  ...
  it("TC - Add project address ", function () {
    _common.select_rowHasValue(cnt.uuid.PROJECTS,PRJ_NO_1)

    _common.openTab(app.TabBar.PROJECT).then(() => {
        _common.select_tabFromFooter(cnt.uuid.PROJECT_ADDRESSES, app.FooterTab.PROJECT_ADRESS_LIST, 0);
      });
          _common.waitForLoaderToDisappear()
          _common.create_newRecord(cnt.uuid.PROJECT_ADDRESSES)
          _common.waitForLoaderToDisappear()
          _common.enterRecord_inNewRow(cnt.uuid.PROJECT_ADDRESSES,app.GridCells.DESCRIPTION,app.InputFields.DOMAIN_TYPE_DESCRIPTION,PRJ_ADD_DESC)
          _common.waitForLoaderToDisappear()
          _common.lookUpButtonInCell(cnt.uuid.PROJECT_ADDRESSES, app.GridCells.ADDRESS_FK, app.InputFields.ICO_INPUT_LOOKUP, 0)
          _common.waitForLoaderToDisappear()
          ...
          _common.waitForLoaderToDisappear()
          _common.waitForLoaderToDisappear()
          _common.select_activeRowInContainer(cnt.uuid.PROJECT_ADDRESSES)
          _common.waitForLoaderToDisappear()
    
          _common.saveCellDataToEnv(cnt.uuid.PROJECT_ADDRESSES, app.GridCells.ADDRESS_FK, "PROJECT_ADDRESS")
  });
    ...
    it("TC - Add Delivery address to contract ", function () {
       ...
       _common.lookUpButtonInCell(cnt.uuid.CONTRACTS, app.GridCells.ADDRESS_ENTITY, app.InputFields.ICO_DOWN, 0)
       _common.waitForLoaderToDisappear()
       _common.edit_dropdownCellWithButton(cnt.uuid.CONTRACTS,app.GridCells.ADDRESS_ENTITY,commonLocators.CommonKeys.GRID,PRJ_ADD_DESC)
   ...
   it("TC - Verify Delivery address in contract ", function () {
     _common.openTab(app.TabBar.CONTRACTS).then(() => {
     _common.select_tabFromFooter(cnt.uuid.CONTRACTS, app.FooterTab.CONTRACTS);
     })
