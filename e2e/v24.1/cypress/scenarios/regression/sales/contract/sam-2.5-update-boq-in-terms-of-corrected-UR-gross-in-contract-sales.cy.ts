...
allure.story("SAM- 2.5 | Update BoQ in terms of Correcter UR(Gross) in Contract Sales.");
describe("SAM- 2.5 | Update BoQ in terms of Correcter UR(Gross) in Contract Sales.", () => {
  before(function () {
    cy.preLoading(
    ...
    cy.fixture("sam/sam-2.5-update-boq-in-terms-of-corrected-ur-gross-in-contract-sales.json").then((data) => {
      this.data = data
      BOQ_PARAMETERS={
  ...
  it("TC - Update Corrected UR(Gross) in Contract Sales module & Update BOQ using wizard. ", function () {   
    ...
    _common.enterRecord_inNewRow(cnt.uuid.BOQ_STRUCTURE1, app.GridCells.PRICE_GROSS, app.InputFields.INPUT_GROUP_CONTENT, CONTAINER_CONTRACTS.GROSS_PRICE)
    cy.SAVE()
    cy.REFRESH_CONTAINER()
    _common.select_rowHasValue(cnt.uuid.BOQ_STRUCTURE1, BOQSTRUCT_DESC)
    _common.getText_fromCell(cnt.uuid.BOQ_STRUCTURE1, app.GridCells.PRICE_GROSS).then(($ele1: JQuery<HTMLElement>) => {
      Cypress.env("CorrecterUR", $ele1.text())
      cy.log(Cypress.env("CorrecterUR"))
    })
    _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD)
    _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.UPDATE_BOQ);
    _boqPage.update_BoQFromWizard(CONTAINER_CONTRACTS.SCOPE, 1)
    cy.SAVE()
    cy.REFRESH_CONTAINER()
  })

  it("TC - Verify Corrected UR(Gross) from Contract Sales gets updated in BOQ.", function () {
    ...
    _common.openTab(app.TabBar.BOQSTRUCTURE).then(() => {
      ...
      _common.select_tabFromFooter(cnt.uuid.BOQ_STRUCTURES, app.FooterTab.BOQ_STRUCTURE, 0);
      _common.setup_gridLayout(cnt.uuid.BOQ_STRUCTURES,CONTAINER_COLUMNS_BOQ_STRUCTURE)
      _common.set_columnAtTop([CONTAINER_COLUMNS_BOQ_STRUCTURE.pricegross], cnt.uuid.BOQ_STRUCTURES)
    });
    ...
    _common.assert_cellData_insideActiveRow(cnt.uuid.BOQ_STRUCTURES, app.GridCells.PRICE_GROSS, Cypress.env("CorrecterUR"))
  })
