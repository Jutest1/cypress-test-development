...
allure.story("SAM- 1.36 | Delivery Address Field using Search button");
describe("SAM- 1.36 | Delivery Address Field using Search button", () => {

  before(function () {
    cy.preLoading(Cypress.env("adminUserName"), Cypress.env("adminPassword"), Cypress.env("parentCompanyName"), Cypress.env("childCompanyName"));
    cy.fixture("sam/sam-1.36-delivery-address-field-using-search-button.json").then((data) => {
      this.data = data;
  ...
  it("TC - Add project address for project_1", function () {
  ...
  it("TC - Create project_2", function () {
    _common.openTab(app.TabBar.PROJECT).then(() => {
        _common.select_tabFromFooter(cnt.uuid.PROJECTS, app.FooterTab.PROJECTS, 0);
      });
      _common.clear_subContainerFilter(cnt.uuid.PROJECTS)
      ...
      _common.waitForLoaderToDisappear()
      _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_SEARCH).delete_pinnedItem()
      _common.search_fromSidebar(commonLocators.CommonKeys.STANDARD, PRJ_NO_2).pinnedItem();
    });
    ...
    it("TC - Add project_1 address to project_2 and verify the address ", function () {
            ...
            _saleContractPage.select_projectsearch_FromLookups_fromModal(cnt.uuid.CONTRACTS,app.GridCells.ADDRESS_ENTITY,app.InputFields.INPUT_GROUP_CONTENT,PRJ_NO_2,MODAL_PROJECTS.ZIP_CODE)
            cy.SAVE()
            _common.waitForLoaderToDisappear()
            _common.assert_cellData_insideActiveRow(cnt.uuid.CONTRACTS,app.GridCells.ADDRESS_ENTITY,Cypress.env("PROJECT_ADDRESS"))
