...
let CONTAINER_COLUMNS_BOQ, CONTAINER_COLUMNS_BOQ_STRUCTURE, CONTAINER_COLUMNS_ESTIMATE, CONTAINER_COLUMNS_LINE_ITEM, CONTAINER_COLUMNS_RESOURCE, CONTAINER_COLUMNS_BID, CONTAINER_COLUMNS_CONTRACT, CONTAINER_COLUMNS_CHANGES, CONTAINER_GENERATE_PAYMENT;

let MODAL_PROJECTS, MODAL_CREATE_BID;

ALLURE.epic("SALES");
ALLURE.feature("Sales-Contract");
ALLURE.story("SAM- 2.20 | Update Payment Schedule Target");

describe("SAM- 2.20 | Update Payment Schedule Target", () => {

    before(function () {
        cy.fixture("sam/sam-2.20-update-payment-schedule-target.json")
            .then((data) => {
                this.data = data;
    ...
    it("TC - Generate Payment Schedule ", function () {
        _common.openTab(app.TabBar.CONTRACTS).then(() => {
        ...
        _common.search_inSubContainer(cnt.uuid.CONTRACTS, CONTRACT_DESC)
        _common.select_rowHasValue(cnt.uuid.CONTRACTS, CONTRACT_DESC)
        _saleContractPage.generate_Payment_Schedule_In_Contract(sidebar.SideBarOptions.GENERATE_PAYMENT_SCHEDULE, _common.getDate(commonLocators.CommonKeys.CURRENT_SMALL), _common.getDate(commonLocators.CommonKeys.INCREMENTED_SMALL, 3), CONTAINER_GENERATE_PAYMENT.RADIO_BUTTON, CONTAINER_GENERATE_PAYMENT.SCHEDULE_TARGATE)
    });

    it("TC - Verify 'Net (OC)' gets updated in 'Payment schedule' container (Net amount = Main contract + change order contract", function () {
        _common.openTab(app.TabBar.CONTRACTS).then(() => {
            _common.select_tabFromFooter(cnt.uuid.CONTRACTS, app.FooterTab.CONTRACTS, 0);
        })
        _common.clear_subContainerFilter(cnt.uuid.CONTRACTS)
        _validate.assertAndVerify_SumOfMultipleCellValues(cnt.uuid.CONTRACTS, app.GridCells.AMOUNT_NET, cnt.uuid.PAYMENT_SCHEDULE_V1, app.GridCells.AMOUNT_NET_OC)
    });

    it("TC - Verify Variance gets updated to zero.", function () {
        _common.openTab(app.TabBar.CONTRACTS).then(() => {
            _common.select_tabFromFooter(cnt.uuid.PAYMENT_SCHEDULE_V1, app.FooterTab.PAYMENT_SCHEDULE, 1);
        })
        _common.clear_subContainerFilter(cnt.uuid.CONTRACTS)
        _common.select_rowHasValue(cnt.uuid.PAYMENT_SCHEDULE_V1, _common.getDate(commonLocators.CommonKeys.CURRENT_SMALL))
        _common.assert_forNumericValues(cnt.uuid.PAYMENT_SCHEDULE_V1, app.GridCells.REMAINING, "0")
    });
})
