...
let CONTAINERS_QUANTITY_TAKEOFF_DETAIL;
let CONTAINER_COLUMNS_QUANTITY_TAKEOFF_DETAIL;
allure.epic("SALES");
allure.feature("Sales-Bill");
allure.story("SAM- 1.58 | Create QTO Detail in container Quantity Takeoff");
describe("SAM- 1.58 | Create QTO Detail in container Quantity Takeoff", () => {
  ...
  before(function () {
    ...
    cy.fixture("sam/sam-1.58-create-qto-detail-in-quantity-takeoff-container.json").then((data) => {
      this.data = data;
      ...
      CONTAINERS_QUANTITY_TAKEOFF_DETAIL = this.data.CONTAINERS.QUANTITY_TAKEOFF_DETAIL
  ...
  it("TC - Create quantity takeoff and verify result", function () {
    ...
    cy.wait(1000)//required wait
    _common.assert_cellData_insideActiveRow(cnt.uuid.BILL_OF_QUANTITY_LOOKUP, app.GridCells.INSTALLED_QUANTITY, CONTAINERS_BILL_OF_QUANTITY_LOOKUP.QUANTITY)
    _common.assert_cellData_insideActiveRow(cnt.uuid.QUANTITY_TAKEOFF_DETAIL,app.GridCells.RESULT,CONTAINERS_BILL_OF_QUANTITY_LOOKUP.QUANTITY) 
