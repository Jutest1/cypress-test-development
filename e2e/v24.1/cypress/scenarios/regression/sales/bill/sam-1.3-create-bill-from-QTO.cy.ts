...
describe("SAM- 1.3 | Create bill from QTO", () => {
  ...
  it("TC - Verify quantity and final price ",function (){
    ...
    _common.set_columnAtTop([CONTAINER_COLUMNS_BOQ_STRUCTURE.boqlinetypefk],cnt.uuid.BILLBOQSTRUCTURE)
    _common.clickOn_cellHasUniqueValue(cnt.uuid.BILLBOQSTRUCTURE,app.GridCells.BOQ_LINE_TYPE_FK,CommonLocators.CommonKeys.POSITION)
    ...
    _validate.verify_isRecordMultiplyTwoValuesInRow(cnt.uuid.BILLBOQSTRUCTURE,CONTAINERS_BILL_OF_QUANTITY_LOOKUP.QUANTITY,CONTAINERS_BOQ_STRUCTURE.UNIT_RATE,app.GridCells.FINAL_PRICE_SMALL)
  })
