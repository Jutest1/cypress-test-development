...
allure.story("SAM- 1.8 | Bill creation in Bill module without WIP");
describe("SAM- 1.8 | Bill creation in Bill module without WIP", () => {
  beforeEach(function () {
    cy.fixture("sam/sam-1.8-Bill-creation-in-Bill-module-without-WIP.json").then((data) => {
      this.data = data;
    });
  });
...
_common.saveCellDataToEnv(cnt.uuid.BIDS,app.GridCells.AMOUNT_NET,"FINAL_PRICE")
  ...
  it("TC - Create bill for Single Invoice without WIP", function () {
    _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD);
    ...
    _billPage.verifyValue_inQuantityBill(CONTAINERS_BILL.BOQ_TYPE)
