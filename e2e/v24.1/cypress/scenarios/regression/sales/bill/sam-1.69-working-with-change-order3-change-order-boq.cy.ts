...
ALLURE.story("SAM- 1.69 | Working with change order 3 - change order boq");
describe("SAM- 1.69 | Working with change order 3 - change order boq", () => {

    before(function () {
        cy.fixture("sam/sam-1.69-working-with-change-order3-change-order-boq.json")
          .then((data) => {
            this.data = data;
    ...
    it("TC - Create QTO and verify In QTO2,  'IQ Quantity' in the Bill Of Container gets reflected from the 'Result' value in the 'Quantity Takeoff' Container", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART);
        _common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.QTO);
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.QTOHEADER).then(() => {
    ...
    it("TC - Create WIP2 from contract and verify WIP2 Net Amount= WIP2 BoQ Qty * Unit Rate", function () {
    ...
    it("TC - Bill1 Net Amount == Summation of Net Amount of both WIPs ( WIP1+WIP2)", function () {
    
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD);
        _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.CREATE_BILL);
        _billPage.create_bill_fromWizard(BILL_PARAMETERS)
        ...
        _common.assert_forNumericValues(cnt.uuid.BILLS,app.GridCells.AMOUNT_NET,totalNetAmount.toString())
    })
});