...
allure.story("SAM- 2.12 | Scenario - QTO: QTO Export Document using 'Entire Quantity Takeoff line' and REB version as 'DA11'");
describe("SAM- 2.12 | Scenario - QTO: QTO Export Document using 'Entire Quantity Takeoff line' and REB version as 'DA11", () => {

  before(function () {
    cy.preLoading(Cypress.env("adminUserName"), Cypress.env("adminPassword"), Cypress.env("parentCompanyName"), Cypress.env("childCompanyName"));
    cy.fixture("sam/sam-2.12-QTO-Export-Document-using-Entire-Quantity-Takeofflinea-nd-REB-version-as-DA11.json").then((data) => {
      this.data = data;
  ...
  it("TC- QTO Export Document for 'Entire Quantity Takeoff line' and REB version as 'DA11", function () {
    _common.openTab(app.TabBar.QTOHEADER).then(() => {
      _common.select_tabFromFooter(cnt.uuid.QUANTITY_TAKEOFF_HEADER, app.FooterTab.QUANTITYTAKEOFFHEADER)
      _common.clear_subContainerFilter(cnt.uuid.QUANTITY_TAKEOFF_HEADER)
    })
    _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_NEW_WIZARD)
    _common.search_fromSidebar(commonLocators.CommonKeys.WIZARD, sidebar.SideBarOptions.EXPORT_QTO_DOCUMENT);
    _bidPage.ExportQTODocument(MODAL_REB_EXPORT.RADIO_LABLE_NAME, MODAL_REB_EXPORT.INDEX, MODAL_REB_EXPORT.REB_VERSION);
    _common.waitForLoaderToDisappear()
  });
