...
import { _common, _estimatePage, _mainView, _modalView, _sidebar, _validate, _boqPage, _bidPage, _projectPage } from "cypress/pages";
...
const LINE_ITEM_DESC_1="BSD1-" + Cypress._.random(0, 999);
const LINE_ITEM_DESC_2="BSD2-" + Cypress._.random(0, 999);
...
ALLURE.story("SAM- 2.4 | Create separate Bids for line items linked with each project change/s and assign it to Main Bid");

describe("SAM- 2.4 | Create separate Bids for line items linked with each project change/s and assign it to Main Bid", () => {
  
  before(function () {
    cy.fixture("sam/sam-2.4-create-seperate-bids-for-line-items-linked-with-each-project-change-and-assign-to-main-bid.json")
      .then((data) => {
        this.data = data;
        this.data = data
  ...
  it("TC - Create new sales bid for project change", function () {
    ...
    _bidPage.createBidRecord_byWizardOptions(MODAL_CREATE_BID.CHANGE_BID_WITH_MULTIPLE_PROJECT_CHANGE,BID_DESC_1,MODAL_CREATE_BID.BUSINESS_PARTNER,MODAL_CREATE_BID.STRUCTURE_TYPE,BID_DESC,"NA","NA",commonLocators.CommonKeys.SEPARATE_BID_FOR_EACH_CHANGE,PROJECT_CHANGES)
    _common.waitForLoaderToDisappear()
  ...
  it("TC - Verifying the cost price with final price  ", function () {
    ...
    _common.waitForLoaderToDisappear()
    _common.select_rowHasValue_onIndexBased(cnt.uuid.BIDS,BID_DESC_1,0)
    ...
    _common.search_inSubContainer(cnt.uuid.BIDS,BID_DESC_1)
    _common.waitForLoaderToDisappear()
    _common.select_rowHasValue_onIndexBased(cnt.uuid.BIDS,BID_DESC_1,1)
