...
let CONTAINER_COLUMNS_BOQS,
  ...
  CONTAINER_COLUMNS_BOQSTRUCTURE_BID

allure.epic("SALES");
allure.feature("Sales-BID");
allure.story("SAM- 2.32 | update-bid-AQ-quantity-from-Estimate-AQ-quantity")

describe("SAM- 2.32 | update-bid-AQ-quantity-from-Estimate-AQ-quantity", () => {
  before(function () {
    cy.fixture("sam/sam-2.32_update-bid-AQ-quantity-from-Estimate-AQ-quantity.json").then((data) => {
      this.data = data;
      ...
      CONTAINER_COLUMNS_BOQSTRUCTURE_BID = this.data.CONTAINER_COLUMNS.BOQSTRUCTURE_BID
      PROJECTS_PARAMETERS = {
  ...
  it("TC - updating AQ quantity", function () {
    ...
    _common.openTab(app.TabBar.ESTIMATELINEITEM).then(() => {
      ...
      _common.setup_gridLayout(cnt.uuid.ESTIMATE_LINEITEMS, CONTAINER_COLUMNS_LINE_ITEM)
      _common.set_columnAtTop([CONTAINER_COLUMNS_LINE_ITEM.estqtyrelboqfk, CONTAINER_COLUMNS_LINE_ITEM.quantity, CONTAINER_COLUMNS_LINE_ITEM.quantitytarget, CONTAINER_COLUMNS_LINE_ITEM.wqquantitytarget], cnt.uuid.ESTIMATE_LINEITEMS)
    });
    _common.waitForLoaderToDisappear()
  ...
  it("TC - updating AQ quantity and create updatd bid  ", function () {
    ...
    _common.assert_cellData_insideActiveRow(cnt.uuid.BOQ_ESTIMATEBYBOQ, app.GridCells.QUANTITY_ADJ_CAPS, Cypress.env(AQ_QUANTITY_ITEM))
    cy.SAVE()
    _common.waitForLoaderToDisappear()
  ...
  it("TC - Verify updated AQ quantity with create bid AQ quantity--Assertion 2", function () {
    _common.openTab(app.TabBar.BIDBOQ).then(() => {
      ...
      _common.setup_gridLayout(cnt.uuid.BOQ_STRUCTUREBID, CONTAINER_COLUMNS_BOQSTRUCTURE_BID)
    });
    _common.clickOn_cellHasUniqueValue(cnt.uuid.BOQ_STRUCTUREBID, app.GridCells.BRIEF_INFO_SMALL, BOQ_STRUCTURE_DESC)
    _common.assert_cellData_insideActiveRow(cnt.uuid.BOQ_STRUCTUREBID, app.GridCells.QUANTITY_ADJ, Cypress.env(AQ_QUANTITY_ITEM))
  });
