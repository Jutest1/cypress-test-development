...
describe("GCC- 1.3 | Create Additional Expenses for Controlling Unit in General Contractor Controlling.", () => {
    ...
    it("TC - Create Additional Expenses for Controlling Units and and Verify them.", function () {
        const STANDARD_INPUTS = this.data.Prerequisites.SidebarInputs;
        const WIZARD_INPUTS = this.data.Prerequisites.Wizards
        const ADDEXP_INPUTS = this.data.GeneralContractorControlling.AdditionalExpenses_Input
        const ADDEXP_GRID = this.data.GeneralContractorControlling.AdditionalExpenses_Column_Headers

        _common.select_rowHasValue(cnt.uuid.Cost_Control, Cypress.env("Item1_CU"))
        _common.openSidebarOption(STANDARD_INPUTS.inWizard)
        _common.search_fromSidebar(STANDARD_INPUTS.wizard, WIZARD_INPUTS.createAdditionalExpenses)
        _common.waitForLoaderToDisappear()
        _common.findInputFieldInsideModalWithIndex(commonLocators.CommonElements.ROW, commonLocators.CommonLabels.DESCRIPTION, 0, app.InputFields.DOMAIN_TYPE_DESCRIPTION).clear({ force: true }).type(ADD_EXP_01, { force: true })
        _common.findInputFieldInsideModalWithIndex(commonLocators.CommonElements.ROW, generic.locators.AMOUNT, 0, app.InputFields.INPUT_GROUP_CONTENT).clear({ force: true }).type(ADDEXP_INPUTS.additionalExp01, { force: true })
        _common.clickOn_modalFooterButton(btn.ButtonText.OK)
        _common.clickOn_modalFooterButton(btn.ButtonText.OK)
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        ...
        _common.select_rowHasValue(cnt.uuid.Cost_Control, Cypress.env("Item1_CU"))
        cy.wait(1000).then(() => {
            _common.assert_forNumericValues(cnt.uuid.Cost_Control, app.gridCells.ADDITIONAL_EXPENSES, ADDEXP_INPUTS.additionalExp01)
        })
        _common.waitForLoaderToDisappear()
        _common.openTab(app.tabBar.GENERAL_CONTRACTOR_CONTROLLING).then(() => {
            _common.select_tabFromFooter(cnt.uuid.Additional_Expenses, app.FooterTab.ADDITIONAL_EXPENSES, 0);
            _common.waitForLoaderToDisappear()
            _common.setup_gridLayout(cnt.uuid.Additional_Expenses, ADDEXP_GRID)
        });
        _common.waitForLoaderToDisappear()
        ...
        cy.wait(500).then(() => {
            ...
            _common.assert_forNumericValues(cnt.uuid.Additional_Expenses, app.GridCells.AMOUNT, ADDEXP_INPUTS.additionalExp01)
        })
        _common.waitForLoaderToDisappear()
        _common.openTab(app.tabBar.GENERAL_CONTRACTOR_CONTROLLING).then(() => {
            _common.select_tabFromFooter(cnt.uuid.Cost_Control, app.FooterTab.COST_CONTROL, 0);
        ...
        _common.search_fromSidebar(STANDARD_INPUTS.wizard, WIZARD_INPUTS.createAdditionalExpenses)
        _common.waitForLoaderToDisappear()
        _common.findInputFieldInsideModalWithIndex(commonLocators.CommonElements.ROW, commonLocators.CommonLabels.DESCRIPTION, 0, app.InputFields.DOMAIN_TYPE_DESCRIPTION).clear({ force: true }).type(ADD_EXP_02, { force: true })
        _common.findInputFieldInsideModalWithIndex(commonLocators.CommonElements.ROW, generic.locators.AMOUNT, 0, app.InputFields.INPUT_GROUP_CONTENT).clear({ force: true }).type(ADDEXP_INPUTS.additionalExp02, { force: true })
        _common.clickOn_modalFooterButton(btn.ButtonText.OK)
        _common.clickOn_modalFooterButton(btn.ButtonText.OK)
        _common.waitForLoaderToDisappear()
        _common.waitForLoaderToDisappear()
        ...
        _common.select_rowHasValue(cnt.uuid.Cost_Control, Cypress.env("Item2_CU"))
        cy.wait(1000).then(() => {
            _common.assert_forNumericValues(cnt.uuid.Cost_Control, app.gridCells.ADDITIONAL_EXPENSES, ADDEXP_INPUTS.additionalExp02)
        })
        _common.openTab(app.tabBar.GENERAL_CONTRACTOR_CONTROLLING).then(() => {
            _common.select_tabFromFooter(cnt.uuid.Additional_Expenses, app.FooterTab.ADDITIONAL_EXPENSES, 0);
        });
        ...
        cy.wait(500).then(() => {
            ...
            _common.assert_forNumericValues(cnt.uuid.Additional_Expenses, app.GridCells.AMOUNT, ADDEXP_INPUTS.additionalExp02)
        })
    })
