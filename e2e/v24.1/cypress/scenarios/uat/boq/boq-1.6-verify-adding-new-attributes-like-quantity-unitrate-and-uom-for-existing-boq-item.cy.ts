...
describe("BOQ- 1.6 | Verify adding new attributes like quantity unit rate and uom for existing boq item", () => {
  ...
  it("TC - Create BoQ Structure", function () {
    ...
    const EDITEDBOQSTRUCTUREINPUTS = this.data.BoqStructure.BoQStructureInputs2
    ...
    cy.SAVE();
    _common.waitForLoaderToDisappear()
    _boqPage.edit_BoQelements(cnt.uuid.BOQ_STRUCTURES,app.GridCells.QUANTITY_SMALL,EDITEDBOQSTRUCTUREINPUTS.Quantity)
    _boqPage.edit_BoQelements(cnt.uuid.BOQ_STRUCTURES, app.GridCells.PRICE,EDITEDBOQSTRUCTUREINPUTS.UnitRate)
    _common.edit_dropdownCellWithInput(cnt.uuid.BOQ_STRUCTURES,app.GridCells.BAS_UOM_FK,EDITEDBOQSTRUCTUREINPUTS.Grid,app.InputFields.INPUT_GROUP_CONTENT,EDITEDBOQSTRUCTUREINPUTS.Uom)
    cy.SAVE()
    _common.waitForLoaderToDisappear()
  });
  it("TC - Verify Newly added Qty, UoM, Unit rate", function () {
    const EDITEDBOQSTRUCTUREINPUTS = this.data.BoqStructure.BoQStructureInputs2
    _common.assert_forNumericValues(cnt.uuid.BOQ_STRUCTURES, app.GridCells.QUANTITY_SMALL,EDITEDBOQSTRUCTUREINPUTS.Quantity)
    _common.assert_forNumericValues(cnt.uuid.BOQ_STRUCTURES,  app.GridCells.PRICE,EDITEDBOQSTRUCTUREINPUTS.UnitRate)
    _common.assert_forNumericValues(cnt.uuid.BOQ_STRUCTURES, app.GridCells.BAS_UOM_FK,EDITEDBOQSTRUCTUREINPUTS.Uom)
  })
