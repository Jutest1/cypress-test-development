...
describe("SAM- 1.66 | Change WIP Quantity", () => {
    ...
    it("TC - Create quantity TakeOff Detail", function () {
        ...
        const QTYTAKEOFFSINPUT = this.data.qtyTakeOffs.inputs;
    ...
    it("TC - Create WIP and Verify Wip Boq Quantity should come from QTO IQ Quantity & WIP BoQ Final Price= Quantity* Unit Rate", function () {
        ...
        _common.openTab(app.TabBar.WIPBOQ).then(() => {
            ...
            _common.setup_gridLayout(cnt.uuid.BOQ_STRUCTUREWIP, BOQSTRUCTUREGRID)   
        });
        _common.waitForLoaderToDisappear()
    ...
    it("TC - Verify change wip quantity", function () {
        ...
        _common.openTab(app.TabBar.WIPBOQ).then(() => {
            ...
            _common.setup_gridLayout(cnt.uuid.WIP_QUANTITY_TAKEOFF, QTYTAKEOFFSGRID)
        });
        _common.select_rowInContainer(cnt.uuid.WIP_QUANTITY_TAKEOFF)
        _common.set_cellCheckboxValue(cnt.uuid.WIP_QUANTITY_TAKEOFF, app.gridCells.ISREADONLY, "uncheck")
        cy.SAVE()
        _common.enterRecord_inNewRow(cnt.uuid.WIP_QUANTITY_TAKEOFF, app.GridCells.VALUE_1_DETAIL, app.InputFields.DOMAIN_TYPE_REMARK, QTYTAKEOFFSINPUT.updatedQuantity)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        ...
        _common.clickOn_cellHasIcon(cnt.uuid.BOQ_STRUCTUREWIP, app.GridCells.TREE, app.GridCellIcons.ICO_BOQ_ITEM)
        _common.assert_cellData_insideActiveRow(cnt.uuid.BOQ_STRUCTUREWIP, app.GridCells.QUANTITY_SMALL, QTYTAKEOFFSINPUT.updatedQuantity)
