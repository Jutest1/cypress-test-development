...
const PLANT_CODE = "APLANT_CODE" + Cypress._.random(0, 999);
...
ALLURE.feature("Workshop (Inspection & Maintenance)");
ALLURE.story("LRM- 1.16 | Verify the execution of job card in job card Module");

describe("LRM- 1.16 | Verify the execution of job card in job card Module", () => {

    
    before(function () {
        
        cy.fixture("LRM/lrm-1.16-verify-the-execution-of-job-card-in-JOB-card-module.json").then((data) => {
            this.data = data;
    ...
    it('TC - Change job card status', function () {
        ...
        cy.REFRESH_CONTAINER();
        _common.waitForLoaderToDisappear()
        _common.search_inSubContainer(cnt.uuid.CARD,PLANT_CODE)
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.CARD,PLANT_CODE)
        ...
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.JOB_CARDS_RECORDS,PLANT_CODE)
