...
const PLANT_COMP_DESC = "Escavator" + Cypress._.random(0, 999);
...
const COMPONENT_MAINTENANCE_SCHEMA_DESC = "CMS-Desc" + Cypress._.random(0, 999);
...
let CONTAINERS_COMPONENT_MAINTENANCE_SCHEMAS;
...
let COMPONENT_MAINTENANCE_SCHEMAS_PARAMETERS;


// ALLURE ANNOATION FOR REPORTS ----------------------------------------------------------------
ALLURE.epic("LOGISTICS AND RESOURCE MANAGEMENT");
ALLURE.feature("Plant Master Data in Life Cycle");
ALLURE.story("LRM- 1.18 | Verify adding components maintenance schema in plant master Module");

describe("LRM- 1.18 | Verify adding components maintenance schema in plant master Module", () => {

    
    before(function () {
        
        cy.fixture("LRM/lrm-1.18-verify-adding-components-maintenance-schema-in-plant-master-module.json").then((data) => {
            this.data = data;
            ...
            PLANT_GROUP_PARAMETER = {
                ...
                [app.GridCells.DESCRIPTION_INFO]: PLANT_GROUP_DESC,
            ...
            CONTAINERS_COMPONENT_MAINTENANCE_SCHEMAS = this.data.CONTAINERS.COMPONENT_MAINTENANCE_SCHEMAS;
            COMPONENT_MAINTENANCE_SCHEMAS_PARAMETERS = {
                [app.GridCells.MAINT_SCHEMA_FK]: CONTAINERS_COMPONENT_MAINTENANCE_SCHEMAS.MAINTENANCE_SCHEMA,
                [app.GridCells.DESCRIPTION]: COMPONENT_MAINTENANCE_SCHEMA_DESC,
                [app.GridCells.VALID_FROM]: CONTAINERS_COMPONENT_MAINTENANCE_SCHEMAS.VALID_FROM,
                [app.GridCells.VALID_TO]:CONTAINERS_COMPONENT_MAINTENANCE_SCHEMAS.VALID_TO, 
            }
    ...
    it('TC - Verify Add Component And Generate Maintenance Records', function () {
        ...
        _common.openTab(app.TabBar.MAINTENANCE).then(() => {
            _common.select_tabFromFooter(cnt.uuid.COMPONENT_MAINTENANCE_SCHEMAS, app.FooterTab.COMPONENT_MAINTENANCE_SCHEMAS)
            _common.clear_subContainerFilter(cnt.uuid.COMPONENT_MAINTENANCE_SCHEMAS)
        });
        _common.create_newRecord(cnt.uuid.COMPONENT_MAINTENANCE_SCHEMAS)
        _logesticPage.enterRecord_toCreateComponentMaintenanceSchemas(cnt.uuid.COMPONENT_MAINTENANCE_SCHEMAS,COMPONENT_MAINTENANCE_SCHEMAS_PARAMETERS)
        _common.waitForLoaderToDisappear()
        cy.SAVE()
        _common.waitForLoaderToDisappear()
    })
