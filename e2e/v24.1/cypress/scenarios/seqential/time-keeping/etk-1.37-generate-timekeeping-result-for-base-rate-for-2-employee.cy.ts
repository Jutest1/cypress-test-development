...
const EMPLOYEE_CODE1='EMPC1-' + Cypress._.random(0, 999);
const EMPLOYEE_DESC1='EMPD1-' + Cypress._.random(0, 999);
...
ALLURE.story("ETK- 1.37 | Generate Timekeeping result for Base rate for 2 employee TK-TR009");

describe("ETK- 1.37 | Generate Timekeeping result for Base rate for 2 employee TK-TR009", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.37-generate-timekeeping-result-for-base-rate-for-2-employee.json").then((data) => {
            this.data = data;
            ...
            ACCOUNT_ALLOCATION_RULES_PARAMETERS={
                [app.GridCells.SURCHARGE_TYPE_FK]: BASE_RATE,
                [app.GridCells.ACCOUNT_COST_FK]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.ACCOUNT_COST,
                [app.GridCells.ACCOUNT_REV_FK]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.ACCOUNT_REV,
                [app.GridCells.NOMINAL_DIMENSION1_COST]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION1_COST,
                [app.GridCells.NOMINAL_DIMENSION2_COST]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION2_COST,
                [app.GridCells.NOMINAL_DIMENSION3_COST]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION3_COST,
                [app.GridCells.NOMINAL_DIMENSION1_REV]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION1_REV,
                [app.GridCells.NOMINAL_DIMENSION2_REV]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION2_REV,
                [app.GridCells.NOMINAL_DIMENSION3_REV]:CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION3_REV    
            },
            ...
            EMPLOYEES_PARAMETERS1={
                [app.GridCells.CODE]: EMPLOYEE_CODE1,
                [app.GridCells.DESCRIPTION_INFO]:EMPLOYEE_DESC1,
                [app.GridCells.START_DATE]: _common.getDate(CONTAINERS_EMPLOYEES.START_DATE),
                [app.GridCells.SHIFT_FK]:SHIFT_MOD_DESC,
                [app.GridCells.TIME_KEEPING_GROUP_FK]:TK_GROUP_CODE,
                [app.GridCells.PAYMENT_GROUP_FK]:PAY_GROUP_DESC,
            ...
            TIMEKEEPING_RESULTS_PARAMETERS1={
                ...
                [app.GridCells.TIME_SYMBOL_FK]:TIME_SYMBOL_CODE,
                [app.GridCells.RATE]:CONTAINERS_TIMEKEEPING_RESULTS.RATE,
                [app.GridCells.SHEET_FK]:EMPLOYEE_CODE1
            },
    ...
    it("TC - Create Data records", function () {
        ...
        _common.search_inSubContainer(cnt.uuid.DATA_RECORDS,CONTAINERS_DATA_TYPES.COST)
        _common.waitForLoaderToDisappear()
        _common. create_newRecordIfDataNotFound(cnt.uuid.DATA_RECORDS,app.GridCells.DESCRIPTION_INFO,CONTAINERS_DATA_TYPES.COST)
        _common.waitForLoaderToDisappear()
        cy.get('@CHECK_DATA')
        .then((value) => {
            ...
            _common.edit_containerCell(cnt.uuid.DATA_RECORDS,app.GridCells.CODE,app.InputFields.DOMAIN_TYPE_CODE,CONTAINERS_DATA_TYPES.COST)
            _common.edit_containerCell(cnt.uuid.DATA_RECORDS,app.GridCells.DESCRIPTION_INFO,app.InputFields.DOMAIN_TYPE_TRANSLATION,CONTAINERS_DATA_TYPES.COST)
            _common.waitForLoaderToDisappear()
            cy.SAVE()
            _common.waitForLoaderToDisappear()
            }
        ...
        _common.waitForLoaderToDisappear()
        _common.clear_subContainerFilter(cnt.uuid.DATA_RECORDS)
        _common.search_inSubContainer(cnt.uuid.DATA_RECORDS,CONTAINERS_DATA_TYPES.REVENUE)
        _common.waitForLoaderToDisappear()
        _common. create_newRecordIfDataNotFound(cnt.uuid.DATA_RECORDS,app.GridCells.DESCRIPTION_INFO,CONTAINERS_DATA_TYPES.REVENUE)
        cy.get('@CHECK_DATA')
        .then((value) => {
            if (value.toString() == "Data does not exist") {
            _common.edit_dropdownCellWithCaret(cnt.uuid.DATA_RECORDS,app.GridCells.LEDGER_CONTEXT_FK,commonLocators.CommonKeys.LIST,CONTAINERS_DATA_TYPES.RIB_DEMO)
            _common.edit_containerCell(cnt.uuid.DATA_RECORDS,app.GridCells.CODE,app.InputFields.DOMAIN_TYPE_CODE,CONTAINERS_DATA_TYPES.REVENUE)
            _common.edit_containerCell(cnt.uuid.DATA_RECORDS,app.GridCells.DESCRIPTION_INFO,app.InputFields.DOMAIN_TYPE_TRANSLATION,CONTAINERS_DATA_TYPES.REVENUE)
            _common.waitForLoaderToDisappear()
            cy.SAVE()
            _common.waitForLoaderToDisappear()
            }
    ...
    it("TC - Create 2 Employees", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
       ...
       _common.waitForLoaderToDisappear()
       _common.create_newRecord(cnt.uuid.EMPLOYEES);
       _timekeepingPage.enterRecord_toCreateEmployees(cnt.uuid.EMPLOYEES,EMPLOYEES_PARAMETERS1) 
       cy.SAVE()
       _common.waitForLoaderToDisappear()
    })
    ...
    it("TC -Create 2 Employee sheets and 2 Timekeeping results", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
        ...
        _common.waitForLoaderToDisappear()
        _common.create_newRecord(cnt.uuid.EMPLOYEE_SHEET)
        _common.edit_dropdownCellWithInput(cnt.uuid.EMPLOYEE_SHEET,app.GridCells.EMPLOYEE_FK,commonLocators.CommonKeys.GRID, app.InputFields.INPUT_GROUP_CONTENT,EMPLOYEE_CODE1)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.REVIEW_RECORDINGS).then(() => {
    ...
    it("TC -Verify timekeeping period transaction result for 2 Employee", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
        _common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.TIMEKEEPING_PERIOD)
        _common.waitForLoaderToDisappear()
        cy.wait(2000)
        ...
        _common.waitForLoaderToDisappear()
        let TRANSACTION_CASE_AMOUNT=(parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE)*parseFloat(CONTAINERS_TIMEKEEPING_RESULTS.HOURS)*parseFloat(CONTAINERS_TIME_SYMBOLS.VAL_PERCENTAGE)/100*parseFloat(CONTAINERS_TIME_SYMBOLS.VAL_RATE))
        ...
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.NOMINAL_DIMENSION1,CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION1_COST)
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.NOMINAL_DIMENSION2,CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION2_COST)
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.NOMINAL_DIMENSION3,CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION3_COST)
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,TRANSACTION_CASE_AMOUNT.toString())
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.NOMINAL_DIMENSION1,CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION1_REV)
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.NOMINAL_DIMENSION2,CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION2_REV)
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.NOMINAL_DIMENSION3,CONTAINERS_ACCOUNT_ALLOCATION_RULES.DIMENSION3_REV)
        _common.waitForLoaderToDisappear()
