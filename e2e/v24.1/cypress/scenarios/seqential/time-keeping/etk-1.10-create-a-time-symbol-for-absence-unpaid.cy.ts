...
ALLURE.story("ETK- 1.10 | Create a Time symbol for Absence (Unpaid)");

describe("ETK- 1.10 | Create a Time symbol for Absence (Unpaid)", () => {
    before(function () {
        cy.fixture("time-keeping/etk-1.10-create-a-time-symbol-for-absence-unpaid.json").then((data) => {
            this.data = data;
    ...
    it("TC - Create time symbols for absence-unpaid", function () {
        ...
        _common.openTab(app.TabBar.TIME_SYMBOLS).then(() => {
            ...
            _common.set_columnAtTop([CONTAINER_COLUMNS_TIME_SYMBOLS.isoffday, CONTAINER_COLUMNS_TIME_SYMBOLS.isvacation, CONTAINER_COLUMNS_TIME_SYMBOLS.valuationpercent], cnt.uuid.TIME_SYMBOLS)
        })
