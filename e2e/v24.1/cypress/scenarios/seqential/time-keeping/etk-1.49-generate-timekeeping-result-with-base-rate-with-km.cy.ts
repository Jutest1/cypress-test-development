...
const TRAVEL_RATE='BA-' + Cypress._.random(0, 999);
...
ALLURE.story("ETK- 1.49 | Generate Timekeeping result with Base rate with Km TK-TR0024");

describe("ETK- 1.49 | Generate Timekeeping result with Base rate with Km TK-TR0024", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.49-generate-timekeeping-result-with-base-rate-with-km.json").then((data) => {
            this.data = data;
            ...
            PAYMENT_GROUP_RATE_PARAMETERS={
                ...
                [app.GridCells.SURCHARGE_TYPE_FK]: TRAVEL_RATE,
    ...
    it("TC - Create Data records", function () {
        ...
        _common.create_newRecord(cnt.uuid.DATA_RECORDS)
        _common.waitForLoaderToDisappear()
        _common.edit_containerCell(cnt.uuid.DATA_RECORDS,app.GridCells.DESCRIPTION_INFO,app.InputFields.DOMAIN_TYPE_TRANSLATION,TRAVEL_RATE)
