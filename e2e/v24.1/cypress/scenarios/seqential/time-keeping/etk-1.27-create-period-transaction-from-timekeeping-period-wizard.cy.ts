...
ALLURE.story("ETK- 1.27 | Create period transaction from timekeeping period wizard");

describe("ETK- 1.27 | Create period transaction from timekeeping period wizard", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.27-create-period-transaction-from-timekeeping-period-wizard.json").then((data) => {
            this.data = data;
    ...
    it("TC - Search for companies and create Timekeeping Groups", function () {
        ...
        _common.waitForLoaderToDisappear()
        _common.clear_subContainerFilter(cnt.uuid.TIMEKEEPING_GROUP)
        _common.waitForLoaderToDisappear()
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.openTab(app.TabBar.PERIOD).then(() => {
            _common.setDefaultView(app.TabBar.PERIOD,"Default")
            _common.select_tabFromFooter(cnt.uuid.PERIOD, app.FooterTab.PERIODS, 0);
        })
        _common.clear_subContainerFilter(cnt.uuid.PERIOD)
