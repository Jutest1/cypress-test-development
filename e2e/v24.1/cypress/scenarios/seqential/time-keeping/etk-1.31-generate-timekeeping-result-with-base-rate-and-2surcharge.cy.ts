...
const CU_SUB_03 = 'SUB-CU3-' + Cypress._.random(0, 999);
const CU_SUB_04 = 'SUB-CU3-' + Cypress._.random(0, 999);
...
const SURCHANGE_RATE='SRR-' + Cypress._.random(0, 999);
const SURCHANGE_RATE1='SRR1-' + Cypress._.random(0, 999);
...
const TIME_SYMBOL_CODE='TSY-C-' + Cypress._.random(0, 999);
const TIME_SYMBOL_DESC='TSY-D-' + Cypress._.random(0, 999);
...
ALLURE.story("ETK- 1.31 | Generate Timekeeping result with Base rate and 2 Surcharge TK-TR004");

describe("ETK- 1.31 | Generate Timekeeping result with Base rate and 2 Surcharge TK-TR004", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.31-generate-timekeeping-result-with-base-rate-and-2surcharge.json").then((data) => {
            this.data = data;
            ...
            CONTROLLING_UNIT_SUB_PARAMETERS_4 = {
                [app.GridCells.DESCRIPTION_INFO]: CU_SUB_04,
                [app.GridCells.QUANTITY_SMALL]: CONTAINERS_CONTROLLING_UNITS.QUANTITY[0],
                [app.GridCells.UOM_FK]: CONTAINERS_CONTROLLING_UNITS.UOM[0]
            },
            ...
            PAYMENT_GROUP_SURCHARGE_PARAMETERS={
                [app.GridCells.CONTROLLING_UNIT_FK]: CU_SUB_03,
                [app.GridCells.SURCHARGE_TYPE_FK]: SURCHANGE_RATE,
                [app.GridCells.COMMENT_TEXT]: COMMENT_TEXT,
                [app.GridCells.RATE]:CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE[0]
            },
            PAYMENT_GROUP_SURCHARGE2_PARAMETERS={
                [app.GridCells.CONTROLLING_UNIT_FK]: CU_SUB_04,
                [app.GridCells.SURCHARGE_TYPE_FK]: SURCHANGE_RATE1,
                [app.GridCells.COMMENT_TEXT]: COMMENT_TEXT,
                [app.GridCells.RATE]:CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE[1]
            },
            ...
            ACCOUNT_ALLOCATION_RULES1_PARAMETERS={
                [app.GridCells.CONTROLLING_UNIT_FK]: CU_SUB_01,
                [app.GridCells.SURCHARGE_TYPE_FK]: SURCHANGE_RATE      
            },
            ACCOUNT_ALLOCATION_RULES2_PARAMETERS={
                [app.GridCells.CONTROLLING_UNIT_FK]: CU_SUB_01,
                [app.GridCells.SURCHARGE_TYPE_FK]: SURCHANGE_RATE1      
            },
            ACCOUNT_ALLOCATION_RULES3_PARAMETERS={
                [app.GridCells.CONTROLLING_UNIT_FK]: CU_SUB_01,
                [app.GridCells.SURCHARGE_TYPE_FK]: BASE_RATE      
            },
    ...
    it("TC - Create Controlling Units", function () {
        ...
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.getText_fromCell(cnt.uuid.CONTROLLING_UNIT,app.GridCells.CODE).then(($C_UNIT3) => {
            Cypress.env("C_UNIT3",($C_UNIT3.text()))
        _common.waitForLoaderToDisappear()   
        })
        _common.select_rowHasValue(cnt.uuid.CONTROLLING_UNIT, CU_MAIN_01)
        _controllingUnit.enterRecord_toCreateControllingUnit(CONTROLLING_UNIT_SUB_PARAMETERS_4)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.getText_fromCell(cnt.uuid.CONTROLLING_UNIT,app.GridCells.CODE).then(($C_UNIT4) => {
            Cypress.env("C_UNIT4",($C_UNIT4.text()))
        _common.waitForLoaderToDisappear()    
        })
    });
    it("TC - Create Data records", function () {
        ...
        _common.create_newRecord(cnt.uuid.DATA_RECORDS)
        _common.waitForLoaderToDisappear()
        _common.edit_containerCell(cnt.uuid.DATA_RECORDS,app.GridCells.DESCRIPTION_INFO,app.InputFields.DOMAIN_TYPE_TRANSLATION,SURCHANGE_RATE1)
        _common.set_cellCheckboxValue(cnt.uuid.DATA_RECORDS,app.GridCells.IS_STANDARDRATE,CONTAINERS_DATA_TYPES.CHECKBOX[1])
        cy.SAVE()
        _common.waitForLoaderToDisappear()
    })
    ...
    it("TC - Create Payment Groups List and Payment Group Rate ", function () {
        ...
        _common.waitForLoaderToDisappear()
        _common.create_newRecord(cnt.uuid.PAYMENT_GROUP_SURCHARGES)
        _timekeepingPage.enterRecord_toPaymentGroupSurcharge(cnt.uuid.PAYMENT_GROUP_SURCHARGES,PAYMENT_GROUP_SURCHARGE2_PARAMETERS)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
    })
    it("TC - Create Time symbols and add Account allocations rules", function () {
       ...
       _timekeepingPage.enterRecord_toAccountAllocationRules(cnt.uuid.ACCOUNT_ALLOCATION_RULES,ACCOUNT_ALLOCATION_RULES1_PARAMETERS)
       cy.SAVE()
       _common.waitForLoaderToDisappear()
       _common.waitForLoaderToDisappear()
       _common.create_newRecord(cnt.uuid.ACCOUNT_ALLOCATION_RULES)
       _timekeepingPage.enterRecord_toAccountAllocationRules(cnt.uuid.ACCOUNT_ALLOCATION_RULES,ACCOUNT_ALLOCATION_RULES2_PARAMETERS)
       cy.SAVE()
       _common.waitForLoaderToDisappear()
       _common.waitForLoaderToDisappear()
       _common.create_newRecord(cnt.uuid.ACCOUNT_ALLOCATION_RULES)
       _timekeepingPage.enterRecord_toAccountAllocationRules(cnt.uuid.ACCOUNT_ALLOCATION_RULES,ACCOUNT_ALLOCATION_RULES3_PARAMETERS)
       cy.SAVE()
       _common.waitForLoaderToDisappear()
    })
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.waitForLoaderToDisappear()
        let TRANSACTION_CASE_AMOUNT1=(parseFloat(CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE[0])/100*parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE))
        _common.waitForLoaderToDisappear()
        let TRANSACTION_CASE_AMOUNT2=(parseFloat(CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE[1])/100*parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE))
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.TRANSACTIONS,Cypress.env("C_UNIT4"))
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.CONTROLLING_UNIT_FK,Cypress.env("C_UNIT4"))
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,TRANSACTION_CASE_AMOUNT2.toString())
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
       _common.select_rowHasValue(cnt.uuid.TRANSACTIONS,Cypress.env("C_UNIT3"))
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.QUANTITY_SMALL,CONTAINERS_TIMEKEEPING_RESULTS.HOURS)
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.CONTROLLING_UNIT_FK,Cypress.env("C_UNIT3"))
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,TRANSACTION_CASE_AMOUNT1.toString())
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.TRANSACTION_CASE,CONTAINERS_TRANSACTIONS.TRANSACTION_CASES[1])
        _common.waitForLoaderToDisappear()
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.TRANSACTION_CASE,CONTAINERS_TRANSACTIONS.TRANSACTION_CASES[1])
        _common.waitForLoaderToDisappear()
        let ADD_AMOUNT=(parseFloat(TRANSACTION_CASE_AMOUNT1.toString())+parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE)+parseFloat(TRANSACTION_CASE_AMOUNT2.toString()))
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,ADD_AMOUNT.toString())
        _common.waitForLoaderToDisappear()
