...
ALLURE.story("ETK- 1.32 | Generate Timekeeping result for Time symbol with is expence flag True TK-TR005");

describe("ETK- 1.32 | Generate Timekeeping result for Time symbol with is expence flag True TK-TR005", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.32-generate-timekeeping-result-for-time-symbol-with-is-expence-flag-true.json").then((data) => {
            this.data = data;
            ...
            TIME_SYMBOLS_PARAMETERS={
                ...
                [app.GridCells.IS_EXPENSE]:  CONTAINERS_TIME_SYMBOLS.CHECKBOX            
            },
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.waitForLoaderToDisappear()
        let TRANSACTION_CASE_AMOUNT=(parseFloat(CONTAINERS_TIMEKEEPING_RESULTS.HOURS)*parseFloat(CONTAINERS_TIME_SYMBOLS.VAL_PERCENTAGE)/100*parseFloat(CONTAINERS_TIME_SYMBOLS.VAL_RATE))
        _common.waitForLoaderToDisappear()
