...
ALLURE.story("ETK- 1.47 | Generate Timekeeping result with Base rate and Surcharge with quantity 16 action TK-TR0022");

describe("ETK- 1.47 | Generate Timekeeping result with Base rate and Surcharge with quantity 16 action TK-TR0022", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.47-generate-timekeeping-result-with-base-rate-and-surchage-with-quantity-16action.json").then((data) => {
            this.data = data;
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.maximizeContainer(cnt.uuid.TRANSACTIONS)
         _common.waitForLoaderToDisappear()
        let AMOUNT1=(parseFloat(CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE)/100*parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE)*parseFloat(CONTAINERS_TIMEKEEPING_RESULTS.HOURS))
        _common.waitForLoaderToDisappear()
        let AMOUNT2=(parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE)*parseFloat(CONTAINERS_TIMEKEEPING_RESULTS.HOURS))
        _common.waitForLoaderToDisappear()
