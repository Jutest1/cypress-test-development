...
ALLURE.story("ETK- 1.44 | Generate timekeeping transaction result for base rate with Employee report TK-TR006");

describe("ETK- 1.44 | Generate timekeeping transaction result for base rate with Employee report TK-TR006", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.44-generate-timekeeping-result-for-base-rate-with-employee-report.json").then((data) => {
            this.data = data;
            ...
            CREW_RECORDING_PARAMETERS = {
              ...
              CONTAINER_COLUMNS_EMPLOYEE_REPORTS=this.data.CONTAINER_COLUMNS.EMPLOYEE_REPORTS
              CONTAINERS_EMPLOYEE_REPORTS=this.data.CONTAINERS.EMPLOYEE_REPORTS
              EMPLOYEE_REPORTS_PARAMETERS={
                  [app.GridCells.DUE_DATE]:_common.getDate(CONTAINERS_EMPLOYEE_REPORTS.DUE_DATE,1),
                  [app.GridCells.TIME_SYMBOL_FK]:TIME_SYMBOL_CODE,
                  [app.GridCells.DURATION]:CONTAINERS_EMPLOYEE_REPORTS.DURATION,
                  [app.GridCells.CONTROLLING_UNIT_FK]:CU_SUB_02,
                  [app.GridCells.JOB_FK]:PRJ_NO,
                  [app.GridCells.PROJECT_ACTION_FK]:ACTIONS_CODE,
              }
        })
    ...
    it("TC -Search for Employee sheets,Employee reports and Create Timekeeping results", function () {
        _common.toggleSidebar(sidebar.sideBarList.SIDEBAR_QUICKSTART)
        ...
        _common.openTab(app.TabBar.REVIEW_RECORDINGS).then(() => {
            ...
            _common.waitForLoaderToDisappear()
            _common.setup_gridLayout(cnt.uuid.EMPLOYEE_REPORT, CONTAINER_COLUMNS_EMPLOYEE_REPORTS)
        })
        _common.create_newRecord(cnt.uuid.EMPLOYEE_REPORT);
        _timekeepingPage.enterRecord_toEmployeeReports(cnt.uuid.EMPLOYEE_REPORT,EMPLOYEE_REPORTS_PARAMETERS)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
        _common.openTab(app.TabBar.REVIEW_RECORDINGS).then(() => {
            _common.set_containerLayoutUnderEditView(commonLocators.CommonKeys.LAYOUT_6)
