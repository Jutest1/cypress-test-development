...
let PAYMENT_GROUP_PARAMETERS:DataCells,PAYMENT_GROUP_RATE_PARAMETERS:DataCells,SHIFT_MODELS_PARAMETERS:DataCells,WORKING_TIMES_PARAMETERS:DataCells,TIME_SYMBOLS_PARAMETERS:DataCells,ACCOUNT_ALLOCATION_RULES_PARAMETERS:DataCells,WORKING_TIME_MODELS_PARAMETERS:DataCells,SCHEDULE_PARAMETERS:DataCells,SCHEDULE_ACTIVITY_PARAMETERS:DataCells,WORKING_TIME_MODEL_DAYS_PARAMETERS:DataCells,TIMEKEEPING_GROUPS_PARAMETERS:DataCells,WORKING_TIME_MODEL_DAYS_PARAMETERS_1:DataCells,WORKING_TIME_MODEL_DAYS_PARAMETERS_2:DataCells,WORKING_TIME_MODEL_DAYS_PARAMETERS_3:DataCells,WORKING_TIME_MODEL_DAYS_PARAMETERS_4:DataCells
let CONTAINER_COLUMNS_PAYMENT_GROUPS,CONTAINER_COLUMNS_ACTIVITY_STRUCTURE,CONTAINER_COLUMNS_SCHEDULES,CONTAINER_COLUMNS_PAYMENT_GROUP_RATES,CONTAINER_COLUMNS_SHIFT_MODELS,CONTAINER_COLUMNS_WORKING_TIMES,CONTAINER_COLUMNS_TIME_SYMBOLS,CONTAINER_COLUMNS_ACCOUNT_ALLOCATION_RULES,CONTAINER_COLUMNS_TIMEKEEPING_GROUPS,CONTAINER_COLUMNS_EMPLOYEES,CONTAINER_COLUMNS_PERIODS

ALLURE.epic("TIME KEEPING");
ALLURE.feature("Time Keeping");
ALLURE.story("ETK- 1.45 | Generate timekeeping transaction result for base rate with activity assigned to action TK-TR0013");

describe("ETK- 1.45 | Generate timekeeping transaction result for base rate with activity assigned to action TK-TR0013", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.45-generate-timekeeping-result-with-base-rate-with-activity-assigned-to-action.json").then((data) => {
            this.data = data;
            ...
            ACTIONS_PARAMETERS = {
                ...
                [app.GridCells.ACTIVITY_FK]:ACTIVITY_STRUCTURE_DESC,
    ...
    it("TC - Create new schedules header and activity", function () {
    ...
    it("TC - Create Data records", function () {
        ...
        _common.set_cellCheckboxValue(cnt.uuid.DATA_RECORDS,app.GridCells.IS_STANDARDRATE,CONTAINERS_DATA_TYPES.CHECKBOX)
        cy.SAVE()
        _common.waitForLoaderToDisappear()
    })
    it("TC - Create Actions with controlling unit", function () {
       ...
       _common.waitForLoaderToDisappear()
       cy.SAVE()
       _common.edit_dropdownCellWithInput(cnt.uuid.PROJECT_ACTIONS, app.GridCells.CONTROLLING_UNIT_FK, commonLocators.CommonKeys.GRID, app.InputFields.INPUT_GROUP_CONTENT,CU_SUB_01)
       _common.waitForLoaderToDisappear()
       cy.SAVE()
       _common.getText_fromCell(cnt.uuid.PROJECT_ACTIONS,app.GridCells.ACTIVITY_FK).then(($C_ACTIVITY) => {
        Cypress.env("ACTIVITY",($C_ACTIVITY.text()))
       })
    _common.waitForLoaderToDisappear() 
    })
    ...
    it("TC -Search for Employee sheets and Create Timekeeping results", function () {
        ...
        _common.openTab(app.TabBar.REVIEW_RECORDINGS).then(() => {
            _common.set_containerLayoutUnderEditView(commonLocators.CommonKeys.LAYOUT_6)
            _common.select_tabFromFooter(cnt.uuid.TIMEKEEPING_RESULTS, app.FooterTab.TIMEKEEPING_RESULTS, 1);
            _common.waitForLoaderToDisappear()
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,CONTAINERS_PAYMENT_GROUP_RATES.RATE)
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.ACTIVITY_FK,Cypress.env("ACTIVITY"))
        _common.waitForLoaderToDisappear()
