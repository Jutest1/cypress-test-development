...
ALLURE.story("ETK- 1.36 | Generate Timekeeping result with Base rate ultiple enteries TK-TR0025");

describe("ETK- 1.36 | Generate Timekeeping result for Base rate multiple enteries TK-TR0025", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.36-generate-timekeeping-result-for-base-rate-multiple-entries.json").then((data) => {
            this.data = data;
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.search_fromSidebar(commonLocators.CommonKeys.QUICKSTART, sidebar.SideBarOptions.TIMEKEEPING_PERIOD)
        _common.waitForLoaderToDisappear()
        cy.wait(1000)//needed to load page
        _common.select_tabFromFooter(cnt.uuid.PERIOD, app.FooterTab.PERIODS);
        _common.waitForLoaderToDisappear()
        _common.clear_subContainerFilter(cnt.uuid.PERIOD)
        ...
        _common.maximizeContainer(cnt.uuid.TRANSACTIONS)
         _common.waitForLoaderToDisappear()
        let HOURS_QUANTITY= (parseFloat(CONTAINERS_TIMEKEEPING_RESULTS.HOURS[0])+parseFloat(CONTAINERS_TIMEKEEPING_RESULTS.HOURS[1]))
        let AMOUNT1=(parseFloat(CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE)/100*parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE)*HOURS_QUANTITY)
        let AMOUNT2=(parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE)*HOURS_QUANTITY)
        let AMOUNT3=(AMOUNT1+AMOUNT2)
        _common.waitForLoaderToDisappear()
        _common.select_rowHasValue(cnt.uuid.TRANSACTIONS,Cypress.env("C_UNIT3"))
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.QUANTITY_SMALL,HOURS_QUANTITY.toString())
        _common.waitForLoaderToDisappear()
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.CONTROLLING_UNIT_FK,Cypress.env("C_UNIT3"))
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,AMOUNT1.toString())
        _common.waitForLoaderToDisappear()
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.QUANTITY_SMALL,HOURS_QUANTITY.toString())
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,AMOUNT2.toString())
        _common.waitForLoaderToDisappear()
        ...
        _common.waitForLoaderToDisappear()
        _common.assert_forNumericValues(cnt.uuid.TRANSACTIONS,app.GridCells.AMOUNT_SMALL,AMOUNT3.toString())
        _common.waitForLoaderToDisappear()
