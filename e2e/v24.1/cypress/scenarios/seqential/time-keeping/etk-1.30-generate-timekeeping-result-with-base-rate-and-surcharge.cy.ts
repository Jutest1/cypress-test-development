...
ALLURE.story("ETK- 1.30 | Generate Timekeeping result with Base rate and Surcharge TK-TR003");

describe("ETK- 1.30 | Generate Timekeeping result with Base rate and Surcharge TK-TR003", () => {

    before(function () {
        cy.fixture("time-keeping/etk-1.30-generate-timekeeping-result-with-base-rate-and-surcharge.json").then((data) => {
            this.data = data;
    ...
    it("TC -Verify timekeeping period transaction result for base rate", function () {
        ...
        _common.maximizeContainer(cnt.uuid.TRANSACTIONS)
         _common.waitForLoaderToDisappear()
        let AMOUNT=(parseFloat(CONTAINERS_PAYMENT_GROUP_SURCHARGE.RATE)/100*parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE))
        _common.waitForLoaderToDisappear()
        ...
        _common.assert_cellData_insideActiveRow(cnt.uuid.TRANSACTIONS,app.GridCells.TRANSACTION_CASE,CONTAINERS_TRANSACTIONS.TRANSACTION_CASES[1])
        _common.waitForLoaderToDisappear()
        let ADD_AMOUNT=(parseFloat(AMOUNT.toString())+parseFloat(CONTAINERS_PAYMENT_GROUP_RATES.RATE))
        _common.waitForLoaderToDisappear()
